...
Pololu::Pololu(const QString & portName, bool debugIn)
{
...
void Pololu::sendTrexCmd(char device, char command, QByteArray data){
	if(debug) qDebug("Sending command 0x%02X to device 0x%02X with data 0x%02X", command, device, data.at(0));
	QByteArray cmd;
	cmd.append(0x80);
...
void Pololu::setMotorSpeed(int motorNum, int motorSpeed){
	...
	if(debug) qDebug() << "setting motor" << motorNum << "to" << motorSpeed;
