...
static void VisionModel_output(int_T tid)
{
  ...
  if (VisionModel_U.ModeSelect == 1.0) {
    ...
    mr_buoy_detector(&VisionModel_U.R_forward_in[0],
                     &VisionModel_U.G_forward_in[0],
                     &VisionModel_U.B_forward_in[0], &rtb_buoy_detectormdl,
    ...
    rtAction = 4;
  } else if (VisionModel_U.ModeSelect == 6.0) {
    rtAction = 5;
  } else {
    rtAction = 6;
  }

  VisionModel_DWork.If_ActiveSubsystem = rtAction;
...
{
  /* Update for If: '<Root>/If' incorporates:
   *  Update for ActionPort: '<S1>/Action Port'
   *  Update for SubSystem: '<Root>/Buoy Detection'
   */
  if (VisionModel_DWork.If_ActiveSubsystem == 2) {
    /* Update for ModelReference: '<S1>/buoy_detector.mdl' */
    mr_buoy_detector_Update(&(VisionModel_DWork.buoy_detectormdl_DWORK1.rtb),
      &(VisionModel_DWork.buoy_detectormdl_DWORK1.rtdw));
  }
...
void VisionModel_initialize(boolean_T firstTime)
{
  ...
  mr_buoy_detector_initialize(1, rtmGetErrorStatusPointer(VisionModel_M),
...
extern "C" void MdlInitializeSizes(void)
{
  ...
  VisionModel_M->Sizes.numBlocks = (55);/* Number of blocks */
...
extern "C" void MdlStart(void)
{
  ...
  VisionModel_DWork.If_ActiveSubsystem = -1;
