...
/*
 * Output and update for atomic system:
 *    '<S55>/Transform Coordinates3'
 *    '<S47>/Transform Coordinates3'
 *    '<S52>/Transform Coordinates3'
...
{
  /* Embedded MATLAB: '<S55>/Transform Coordinates3' */
  /* Embedded MATLAB Function 'validation_gate/If Action Subsystem/Transform Coordinates3': '<S56>:1' */
...
void VisionModel_step(void)
{
  ...
  for (i = 0; i < 19200; i++) {
    ...
    if (VisionModel_B.Gain5[i] > VisionModel_B.Gain4[i]) {
      if (VisionModel_B.Gain4[i] < VisionModel_B.Gain3[i]) {
        ii = VisionModel_B.Gain4[i];
      } else {
        ii = VisionModel_B.Gain3[i];
      }

      if (VisionModel_B.Gain5[i] > VisionModel_B.Gain3[i]) {
        maxL = VisionModel_B.Gain5[i];
      } else {
        maxL = VisionModel_B.Gain3[i];
      }
    } else {
      if (VisionModel_B.Gain5[i] < VisionModel_B.Gain3[i]) {
        ii = VisionModel_B.Gain5[i];
      } else {
        ii = VisionModel_B.Gain3[i];
      }

      if (VisionModel_B.Gain4[i] > VisionModel_B.Gain3[i]) {
        maxL = VisionModel_B.Gain4[i];
      } else {
        maxL = VisionModel_B.Gain3[i];
      }
    }
    ...
    if (ii != 0.0) {
      if (VisionModel_B.Gain5[i] == maxL) {
        positive_area = (VisionModel_B.Gain4[i] - VisionModel_B.Gain3[i]) / ii;
      } else if (VisionModel_B.Gain4[i] == maxL) {
        positive_area = (VisionModel_B.Gain3[i] - VisionModel_B.Gain5[i]) / ii +
          2.0;
      } else {
        positive_area = (VisionModel_B.Gain5[i] - VisionModel_B.Gain4[i]) / ii +
          4.0;
      }
    ...
    /* assign the results */
    VisionModel_B.ColorSpaceConversion1_o1_m[i] = positive_area;
  ...
  for (i = 0; i < 19200; i++) {
    ...
    VisionModel_B.Gain3[i] = VisionModel_P.Gain3_Gain *
      VisionModel_B.ColorSpaceConversion1_o1_m[i];

    /* Gain: '<S5>/Gain4' */
    VisionModel_B.Gain4[i] = VisionModel_P.Gain4_Gain *
      VisionModel_B.ColorSpaceConversion1_o2_e[i];

    /* Gain: '<S5>/Gain5' */
    VisionModel_B.Gain5[i] = VisionModel_P.Gain5_Gain *
      VisionModel_B.ColorSpaceConversion1_o3_jm[i];
  }
  ...
  ii = 0.0;
  for (i = 0; i < 19200; i++) {
    maxL = 6.0 * VisionModel_B.Gain3[i];
    i_gate_idx = (real_T)(uint32_T)(maxL - 1.3322676295501878E-15);
    maxL -= i_gate_idx;
    minL = 1.0 - VisionModel_B.Gain4[i];
    jj = 1.0 - VisionModel_B.Gain4[i] * maxL;
    maxL = VisionModel_B.Gain4[i] * maxL + minL;
    if (i_gate_idx == 0.0) {
      positive_area = 1.0;
      c = maxL;
      ii = minL;
    ...
    maxL = VisionModel_B.Gain5[i] / maxL;

    /* assign the results */
    VisionModel_B.ColorSpaceConversion2_o1[i] = maxL * positive_area;
  ...
  /* '<S9>:1:34' */
  VisionModel_B.a_range[9] = VisionModel_U.orane_au;
  ...
  /* '<S9>:1:45' */
  VisionModel_B.b_range[12] = VisionModel_U.breen_bl;
  ...
  for (i = 0; i < 19200; i++) {
    ...
    /* assign the results */
    VisionModel_B.ColorSpaceConversion3_o1[i] = positive_area;
  ...
  for (i = 0; i < 19200; i++) {
    ...
    /* assign the results */
    VisionModel_B.ColorSpaceConversion4_o1[i] = maxL * positive_area;
  ...
  if ((VisionModel_U.ModeSelect == 1.0) || (VisionModel_U.ModeSelect == 3.0) ||
    ...
    for (i = 0; i < 19200; i++) {
      ...
      if (VisionModel_B.sf_LowPassFilter.Iout[i] <= 0.039286085583733095) {
        maxL = VisionModel_B.sf_LowPassFilter.Iout[i] / 12.923054468333255;
      } else {
        maxL = (VisionModel_B.sf_LowPassFilter.Iout[i] + 0.055000519817226347) /
          1.0550005198172263;
        maxL = rt_pow_snf(maxL, 2.4);
      }

      if (VisionModel_B.sf_LowPassFilter1.Iout[i] <= 0.039286085583733095) {
        ii = VisionModel_B.sf_LowPassFilter1.Iout[i] / 12.923054468333255;
      } else {
        ii = (VisionModel_B.sf_LowPassFilter1.Iout[i] + 0.055000519817226347) /
          1.0550005198172263;
        ii = rt_pow_snf(ii, 2.4);
      }

      if (VisionModel_B.sf_LowPassFilter2.Iout[i] <= 0.039286085583733095) {
        jj = VisionModel_B.sf_LowPassFilter2.Iout[i] / 12.923054468333255;
      } else {
        jj = (VisionModel_B.sf_LowPassFilter2.Iout[i] + 0.055000519817226347) /
          1.0550005198172263;
        jj = rt_pow_snf(jj, 2.4);
      }
      ...
      positive_area = (0.41239079926596 * maxL + 0.35758433938388 * ii) +
        0.18048078840183 * jj;
      c = (0.21263900587151 * maxL + 0.71516867876776 * ii) + 0.07219231536073 *
        jj;
      ii = (0.01933081871559 * maxL + 0.11919477979463 * ii) + 0.95053215224966 *
        jj;

      /* Make sure that the output is in [0..1] range; clip if necessary */
      if (positive_area > 1.0) {
        positive_area = 1.0;
      } else {
        if (positive_area < 0.0) {
          positive_area = 0.0;
      ...
      /* assign the results */
      VisionModel_B.ColorSpaceConversion1_o1_i[i] = positive_area;
    ...
    for (col = 0; col < 160; col++) {
      ...
      for (numElem = 0; numElem < 80; numElem++) {
        ...
        ii =
          VisionModel_B.ColorSpaceConversion1_o1_i[VisionModel_ConstP.pooled14[numElem]
          * 160 + col] * VisionModel_ConstP.pooled6[numElem] +
          VisionModel_B.ColorSpaceConversion1_o1_i[VisionModel_ConstP.pooled14[find_white]
          * 160 + col] * VisionModel_ConstP.pooled6[find_white];
        find_white += 80;
        ii +=
          VisionModel_B.ColorSpaceConversion1_o1_i[VisionModel_ConstP.pooled14[find_white]
          * 160 + col] * VisionModel_ConstP.pooled6[find_white];
    ...
    for (numElem = 0; numElem < 80; numElem++) {
      ...
      for (col = 0; col < 60; col++) {
        ...
        for (imgCol = 0; imgCol < 6; imgCol++) {
          ii += Resize2_LineBuffer_o[VisionModel_ConstP.pooled13[find_white]] *
    ...
    for (col = 0; col < 160; col++) {
      ...
      for (numElem = 0; numElem < 80; numElem++) {
        find_white = numElem + 80;
        ii =
          VisionModel_B.ColorSpaceConversion1_o2_g[VisionModel_ConstP.pooled14[numElem]
          * 160 + col] * VisionModel_ConstP.pooled6[numElem] +
          VisionModel_B.ColorSpaceConversion1_o2_g[VisionModel_ConstP.pooled14[find_white]
          * 160 + col] * VisionModel_ConstP.pooled6[find_white];
        find_white += 80;
        ii +=
          VisionModel_B.ColorSpaceConversion1_o2_g[VisionModel_ConstP.pooled14[find_white]
    ...
    for (col = 0; col < 160; col++) {
      ...
      for (numElem = 0; numElem < 80; numElem++) {
        find_white = numElem + 80;
        ii =
          VisionModel_B.ColorSpaceConversion1_o3_j[VisionModel_ConstP.pooled14[numElem]
          * 160 + col] * VisionModel_ConstP.pooled6[numElem] +
          VisionModel_B.ColorSpaceConversion1_o3_j[VisionModel_ConstP.pooled14[find_white]
          * 160 + col] * VisionModel_ConstP.pooled6[find_white];
        find_white += 80;
        ii +=
          VisionModel_B.ColorSpaceConversion1_o3_j[VisionModel_ConstP.pooled14[find_white]
    ...
    VisionM_ReferenceColorSelection(VisionModel_B.Resize_e,
      ...
      VisionModel_P.Dist_Thresh_Value, &VisionModel_B.sf_ReferenceColorSelection);
    ...
    if (VisionModel_B.Add22[0] == 1.0) {
      ...
      jj = 1.0;
      condition = FALSE;
      while (((uint32_T)condition == 0U) && (jj <=
        ...
        if ((positive_area / 19200.0 >= 0.1) && (jj > 1.0)) {
          /* '<S13>:1:73' */
          condition = TRUE;
        } else {
          /* '<S13>:1:77' */
    ...
    for (maxL = 1.0; maxL <= c; maxL++) {
      ...
      if (condition_0 == 1) {
        ...
        for (numElem = 0; numElem < 160; numElem++) {
          ...
          for (condition_0 = 0; condition_0 < 120; condition_0++) {
            /* '<S13>:1:182' */
            if (VisionModel_B.LabelMatrix_n[160 * condition_0 + numElem] != maxL)
            {
              /* '<S13>:1:183' */
              /* '<S13>:1:184' */
              VisionModel_B.LogicMatrix[numElem + 160 * condition_0] = FALSE;
            } else {
              /* '<S13>:1:186' */
              VisionModel_B.LogicMatrix[numElem + 160 * condition_0] = TRUE;

              /* '<S13>:1:187' */
              VisionModel_B.bw_image_m[numElem + 160 * condition_0] = 1;
            }
          }
        }
        ...
        for (c_i = 1U; c_i <= TNumBlobs; c_i = (uint8_T)((uint32_T)c_i + 1U)) {
          /* '<S13>:1:194' */
          i_gate_idx = floor((r + (real_T)TNumBlobs) + 0.5);
          if (i_gate_idx < 256.0) {
          ...
          if (tmp <= 50) {
            /* '<S13>:1:195' */
            /* '<S13>:1:197' */
            i_gate_idx = floor((r + (real_T)c_i) + 0.5);
            if (i_gate_idx < 256.0) {
              tmp = (uint8_T)i_gate_idx;
            } else {
              tmp = MAX_uint8_T;
    ...
    if (rtb_Add1 > 0.0) {
      ...
      for (condition_0 = 0; condition_0 < 160; condition_0++) {
        ...
        for (current_color = 0; current_color < 120; current_color++) {
          /* '<S24>:1:13' */
          /* '<S24>:1:15' */
          VisionModel_B.Gain5[condition_0 + 160 * current_color] =
            VisionModel_B.sf_ReferenceColorSelection.ref_colors[(int32_T)
            VisionModel_B.LabelMatrix_n[160 * current_color + condition_0] - 1];

          /* '<S24>:1:16' */
          VisionModel_B.Gain4[condition_0 + 160 * current_color] =
            VisionModel_B.sf_ReferenceColorSelection.ref_colors[(int32_T)
            VisionModel_B.LabelMatrix_n[160 * current_color + condition_0] + 49];

          /* '<S24>:1:17' */
          VisionModel_B.Gain3[condition_0 + 160 * current_color] =
            VisionModel_B.sf_ReferenceColorSelection.ref_colors[(int32_T)
      ...
      for (i = 0; i < 19200; i++) {
        c = (VisionModel_B.Gain5[i] + 16.0) / 116.0;
        positive_area = VisionModel_B.Gain4[i] / 500.0 + c;
        ii = VisionModel_B.Gain3[i] / -200.0 + c;
        if (positive_area <= 0.20689655172413793) {
          positive_area -= 0.13793103448275862;
          positive_area /= 7.7870370370370372;
        } else {
          positive_area = rt_pow_snf(positive_area, 3.0);
        }
        ...
        positive_area *= 0.9504;
        ii *= 1.0889;

        /* assign the results */
        VisionModel_B.ColorSpaceConversion_o1_m[i] = positive_area;
      ...
      for (i = 0; i < 19200; i++) {
        ...
        /* BT.709 reference primaries for sRGB and CIE standard illuminant */
        /* D65, 2 degree observer */
        positive_area = (3.24096994190451 *
                         VisionModel_B.ColorSpaceConversion_o1_m[i] +
                         -1.53738317757009 *
                         VisionModel_B.ColorSpaceConversion_o2_a[i]) +
        ...
        if (positive_area <= 0.00304) {
          positive_area *= 12.923054468333255;
        } else {
          positive_area = 1.0550005198172263 * rt_pow_snf(positive_area,
    ...
    if ((VisionModel_U.ModeSelect == 2.0) || (VisionModel_U.ModeSelect == 6.0))
    {
      ...
      for (i = 0; i < 19200; i++) {
        ...
        if (VisionModel_B.sf_LowPassFilter_f.Iout[i] <= 0.039286085583733095) {
          maxL = VisionModel_B.sf_LowPassFilter_f.Iout[i] / 12.923054468333255;
        } else {
          maxL = (VisionModel_B.sf_LowPassFilter_f.Iout[i] +
                  0.055000519817226347) / 1.0550005198172263;
          maxL = rt_pow_snf(maxL, 2.4);
        }

        if (VisionModel_B.sf_LowPassFilter1_e.Iout[i] <= 0.039286085583733095) {
          ii = VisionModel_B.sf_LowPassFilter1_e.Iout[i] / 12.923054468333255;
        } else {
          ii = (VisionModel_B.sf_LowPassFilter1_e.Iout[i] + 0.055000519817226347)
            / 1.0550005198172263;
          ii = rt_pow_snf(ii, 2.4);
        }

        if (VisionModel_B.sf_LowPassFilter2_n.Iout[i] <= 0.039286085583733095) {
          jj = VisionModel_B.sf_LowPassFilter2_n.Iout[i] / 12.923054468333255;
        } else {
          jj = (VisionModel_B.sf_LowPassFilter2_n.Iout[i] + 0.055000519817226347)
            / 1.0550005198172263;
          jj = rt_pow_snf(jj, 2.4);
        }
        ...
        c = (0.21263900587151 * maxL + 0.71516867876776 * ii) + 0.07219231536073
          * jj;
        ii = (0.01933081871559 * maxL + 0.11919477979463 * ii) +
          0.95053215224966 * jj;
        ...
        /* assign the results */
        VisionModel_B.ColorSpaceConversion1_o1[i] = positive_area;
      ...
      for (col = 0; col < 120; col++) {
        ...
        for (numElem = 0; numElem < 80; numElem++) {
          find_white = numElem + 80;
          ii =
            VisionModel_B.ColorSpaceConversion1_o1[VisionModel_ConstP.pooled16[numElem]
            * 120 + col] * VisionModel_ConstP.pooled8[numElem] +
            VisionModel_B.ColorSpaceConversion1_o1[VisionModel_ConstP.pooled16[find_white]
            * 120 + col] * VisionModel_ConstP.pooled8[find_white];
          find_white += 80;
          ii +=
            VisionModel_B.ColorSpaceConversion1_o1[VisionModel_ConstP.pooled16[find_white]
            * 120 + col] * VisionModel_ConstP.pooled8[find_white];
      ...
      for (numElem = 0; numElem < 80; numElem++) {
        ...
        for (col = 0; col < 60; col++) {
          find_white = col + 60;
          ii = Resize2_LineBuffer[VisionModel_ConstP.pooled15[col]] *
            VisionModel_ConstP.pooled7[col] +
            Resize2_LineBuffer[VisionModel_ConstP.pooled15[find_white]] *
            VisionModel_ConstP.pooled7[find_white];
          find_white += 60;
          ii += Resize2_LineBuffer[VisionModel_ConstP.pooled15[find_white]] *
            VisionModel_ConstP.pooled7[find_white];
          find_white += 60;
          ii += Resize2_LineBuffer[VisionModel_ConstP.pooled15[find_white]] *
            VisionModel_ConstP.pooled7[find_white];
      ...
      for (col = 0; col < 120; col++) {
        ...
        for (numElem = 0; numElem < 80; numElem++) {
          find_white = numElem + 80;
          ii =
            VisionModel_B.ColorSpaceConversion1_o2[VisionModel_ConstP.pooled16[numElem]
            * 120 + col] * VisionModel_ConstP.pooled8[numElem] +
            VisionModel_B.ColorSpaceConversion1_o2[VisionModel_ConstP.pooled16[find_white]
            * 120 + col] * VisionModel_ConstP.pooled8[find_white];
          find_white += 80;
          ii +=
            VisionModel_B.ColorSpaceConversion1_o2[VisionModel_ConstP.pooled16[find_white]
      ...
      for (col = 0; col < 120; col++) {
        ...
        for (numElem = 0; numElem < 80; numElem++) {
          find_white = numElem + 80;
          ii =
            VisionModel_B.ColorSpaceConversion1_o3[VisionModel_ConstP.pooled16[numElem]
            * 120 + col] * VisionModel_ConstP.pooled8[numElem] +
            VisionModel_B.ColorSpaceConversion1_o3[VisionModel_ConstP.pooled16[find_white]
            * 120 + col] * VisionModel_ConstP.pooled8[find_white];
          find_white += 80;
          ii +=
            VisionModel_B.ColorSpaceConversion1_o3[VisionModel_ConstP.pooled16[find_white]
      ...
      VisionM_ReferenceColorSelection(VisionModel_B.Resize,
        ...
        VisionModel_P.Dist_Thresh_Value_k,
      ...
      for (maxL = 1.0; maxL <= c; maxL++) {
        ...
        if (condition_0 == 1) {
          ...
          for (numElem = 0; numElem < 120; numElem++) {
            ...
            for (condition_0 = 0; condition_0 < 160; condition_0++) {
              /* '<S25>:1:182' */
              if (VisionModel_B.LabelMatrix[120 * condition_0 + numElem] != maxL)
              {
                /* '<S25>:1:183' */
                /* '<S25>:1:184' */
                VisionModel_B.LogicMatrix[numElem + 120 * condition_0] = FALSE;
              } else {
                /* '<S25>:1:186' */
                VisionModel_B.LogicMatrix[numElem + 120 * condition_0] = TRUE;

                /* '<S25>:1:187' */
                VisionModel_B.bw_image_m[numElem + 120 * condition_0] = 1;
              }
            }
          }
      ...
      if (rtb_Add1 > 0.0) {
        ...
        for (condition_0 = 0; condition_0 < 120; condition_0++) {
          ...
          for (current_color = 0; current_color < 160; current_color++) {
            /* '<S36>:1:13' */
            /* '<S36>:1:15' */
            VisionModel_B.Gain5[condition_0 + 120 * current_color] =
              VisionModel_B.sf_ReferenceColorSelection_e.ref_colors[(int32_T)
              VisionModel_B.LabelMatrix[120 * current_color + condition_0] - 1];

            /* '<S36>:1:16' */
            VisionModel_B.Gain4[condition_0 + 120 * current_color] =
              VisionModel_B.sf_ReferenceColorSelection_e.ref_colors[(int32_T)
              VisionModel_B.LabelMatrix[120 * current_color + condition_0] + 49];

            /* '<S36>:1:17' */
            VisionModel_B.Gain3[condition_0 + 120 * current_color] =
              VisionModel_B.sf_ReferenceColorSelection_e.ref_colors[(int32_T)
        ...
        for (i = 0; i < 19200; i++) {
          ...
          /* assign the results */
          VisionModel_Y.R_down_out[i] = positive_area;
        ...
        for (i = 0; i < 19200; i++) {
          ...
          /* BT.709 reference primaries for sRGB and CIE standard illuminant */
          /* D65, 2 degree observer */
          positive_area = (3.24096994190451 * VisionModel_Y.R_down_out[i] +
  ...
  if (VisionModel_U.ModeSelect == 1.0) {
    ...
    maxL = 0.0;
    for (col = 0; col < 6; col++) {
      ii += VisionModel_B.bw_image_e[VisionModel_DWork.EdgeDetection_VOUL_DW[col]]
        * VisionModel_ConstP.pooled9[col];
      jj += VisionModel_B.bw_image_e[VisionModel_DWork.EdgeDetection_HOUL_DW[col]]
        * VisionModel_ConstP.pooled10[col];
    ...
    for (imgCol = 0; imgCol < 120; imgCol++) {
      for (current_color = 0; current_color < 160; current_color++) {
        ...
        VisionModel_B.EdgeDetection[col] =
    ...
    /* Embedded MATLAB Function 'validation_gate/Identify Validation Gate': '<S54>:1' */
    ...
    for (col = 0; col < 200; col++) {
      ...
      for (i = 0; i < 399; i++) {
        ...
        for (condition_0 = 0; condition_0 < 180; condition_0++) {
          ...
          if (VisionModel_B.hough_table[399 * condition_0 + i] > ii) {
            ...
            /* '<S54>:1:73' */
            j_index[col] = (uint8_T)(condition_0 + 1);
      ...
      /* '<S54>:1:79' */
      rho[col] = VisionModel_B.HoughTransform1_o3[i_index[col] - 1];

      /* '<S54>:1:80' */
      theta[col] = VisionModel_B.HoughTransform1_o2[j_index[col] - 1];
    ...
    /* '<S54>:1:106' */
    for (numElem = 0; numElem < 200; numElem++) {
      /* '<S54>:1:106' */
      /*  Consider only lines that are either vertical or horizontal, within */
      /*    tolerance */
      if ((fabs(theta[numElem] - 1.5707963267948966) <= 0.17453292519943295) ||
          (fabs(theta[numElem] + 1.5707963267948966) <= 0.17453292519943295)) {
        /* '<S54>:1:110' */
        condition = TRUE;
      } else {
        /* '<S54>:1:110' */
      ...
      /*  Vertical line */
      guard = FALSE;
      if (fabs(theta[numElem]) <= 0.17453292519943295) {
        /* '<S54>:1:113' */
        if ((rho[numElem] == 0.0) && (theta[numElem] == 0.0)) {
          /* '<S54>:1:113' */
          c_0 = TRUE;
        } else {
          /* '<S54>:1:113' */
        ...
        if (!c_0) {
          ...
          while (((uint32_T)exitg_0 == 0U) && (condition_0 <= 6)) {
            ...
            if (fabs(rho[numElem] - (real_T)rbinv[condition_0 - 1]) <= 16.0) {
              /* '<S54>:1:118' */
              /* '<S54>:1:119' */
              ravgv[condition_0 - 1] = ravgv[condition_0 - 1] + rho[numElem];

              /* '<S54>:1:120' */
              tavgv[condition_0 - 1] = tavgv[condition_0 - 1] + theta[numElem];
      ...
      if (guard && condition) {
        ...
        if (theta[numElem] < 0.0) {
          /* '<S54>:1:129' */
          /* '<S54>:1:130' */
          theta[numElem] = theta[numElem] + 3.1415926535897931;

          /* '<S54>:1:131' */
          rho[numElem] = -rho[numElem];
        }
        ...
        while (((uint32_T)exitg == 0U) && (condition_0 <= 6)) {
          /* '<S54>:1:134' */
          /*  Place each rho value into a bin */
          if (fabs(rho[numElem] - (real_T)rbinh[condition_0 - 1]) <= 16.0) {
            /* '<S54>:1:137' */
            /* '<S54>:1:138' */
            ravgh[condition_0 - 1] = ravgh[condition_0 - 1] + rho[numElem];

            /* '<S54>:1:139' */
            tavgh[condition_0 - 1] = tavgh[condition_0 - 1] + theta[numElem];
    ...
    for (condition_0 = 0; condition_0 < 6; condition_0++) {
      ...
      if (vbinv[condition_0] == 0.0) {
        ...
        /* '<S54>:1:155' */
        i_gate_idx = ravgv[condition_0] / vbinv[condition_0];
      ...
      if (vbinh[condition_0] == 0.0) {
        ...
        /* '<S54>:1:164' */
        L_white = ravgh[condition_0] / vbinh[condition_0];

        /* '<S54>:1:165' */
        L_black = tavgh[condition_0] / vbinh[condition_0];
      }

      ravgv[condition_0] = i_gate_idx;
      tavgv[condition_0] = rtb_Add1;
      ravgh[condition_0] = L_white;
      tavgh[condition_0] = L_black;
    }
    ...
    for (condition_0 = 0; condition_0 < 6; condition_0++) {
      ...
      if ((ravgv[condition_0] < ii) && (ravgv[condition_0] != 0.0)) {
        /* '<S54>:1:176' */
        /* '<S54>:1:177' */
        ii = ravgv[condition_0];

        /* '<S54>:1:178' */
        positive_area = ravgv[condition_0];

        /* '<S54>:1:179' */
        minL = tavgv[condition_0];
      }

      /*  Extract the right-most vertical line */
      if ((ravgv[condition_0] > jj) && (ravgv[condition_0] != 0.0)) {
        /* '<S54>:1:183' */
        /* '<S54>:1:184' */
        jj = ravgv[condition_0];

        /* '<S54>:1:185' */
        rtb_rho_gate_idx = ravgv[condition_0];
    ...
    if (fabs(rtb_rho_gate_idx - positive_area) < 32.0) {
      /* '<S54>:1:193' */
      /* '<S54>:1:194' */
      positive_area = 0.0;
    ...
    for (condition_0 = 0; condition_0 < 6; condition_0++) {
      ...
      if ((vbinh[condition_0] > ii) && (ravgh[condition_0] != 0.0)) {
        ...
        /* '<S54>:1:207' */
        rtb_rho_gate_idx_0 = ravgh[condition_0];

        /* '<S54>:1:208' */
        rtb_theta_gate_idx_0 = tavgh[condition_0];
      }
    ...
    /* '<S54>:1:222' */
    ii = (positive_area / cos(minL) - rtb_rho_gate_idx_0 / cos
          (rtb_theta_gate_idx_0)) * (1.0 / (tan(minL) - tan(rtb_theta_gate_idx_0)));

    /* '<S54>:1:223' */
    jj = (positive_area - ii * sin(minL)) * (1.0 / cos(minL));
    ...
    /* '<S54>:1:236' */
    u[0] = 1.0;
    u[1] = tan((((rtb_theta_gate_idx_0 - 1.5707963267948966) + minL) +
    ...
    if (VisionModel_U.OutputImage > 0.0) {
      ...
      /* S-Function (svipdrawmarkers): '<S55>/Draw Target' */
      /* Copy the image from input to output. */
      for (i = 0; i < 19200; i++) {
        VisionModel_B.Gain5[i] = VisionModel_B.ColorSpaceConversion_o1_m[i];
        VisionModel_B.Gain4[i] = VisionModel_B.ColorSpaceConversion_o2_a[i];
        VisionModel_B.Gain3[i] = VisionModel_B.ColorSpaceConversion_o3_p[i];
      }
      ...
      for (idxROI = 0; idxROI < 2; idxROI += 2) {
        ...
        if (condition) {
          ...
          while (numElem <= line_idx_2) {
            VisionModel_B.Gain5[current_color] =
              VisionModel_P.DrawTarget_RTP_FILLCOLOR[0];
            VisionModel_B.Gain4[current_color] =
              VisionModel_P.DrawTarget_RTP_FILLCOLOR[1];
            VisionModel_B.Gain3[current_color] =
      ...
      /* S-Function (sviphoughlines): '<S55>/Hough Lines' */
      numElem = 0;
      ii = (positive_area + 2.2204460492503131E-16) / (cos(minL) +
      ...
      minL = (positive_area + 2.2204460492503131E-16) / (sin(minL) +
        2.2204460492503131E-16);
      ...
      numElem = 0;
      ii = (rtb_rho_gate_idx_0 + 2.2204460492503131E-16) / (cos
      ...
      minL = (rtb_rho_gate_idx_0 + 2.2204460492503131E-16) / (sin
        (rtb_theta_gate_idx_0) + 2.2204460492503131E-16);
      ...
      numElem = 0;
      ii = (rtb_rho_gate_idx + 2.2204460492503131E-16) / (cos(rtb_theta_gate_idx)
        + 2.2204460492503131E-16);
      ...
      minL = (rtb_rho_gate_idx + 2.2204460492503131E-16) / (sin
      ...
      /* S-Function (svipdrawshapes): '<S55>/Draw Shapes' */
      /* Copy the image from input to output. */
      for (i = 0; i < 19200; i++) {
        VisionModel_B.DrawShapes_o1[i] = VisionModel_B.Gain5[i];
        VisionModel_B.DrawShapes_o2[i] = VisionModel_B.Gain4[i];
        VisionModel_B.DrawShapes_o3[i] = VisionModel_B.Gain3[i];
      }
      ...
      if (VisionModel_U.ModeSelect == 3.0) {
        ...
        for (ii = 1.0; ii <= VisionModel_B.blob_count_c; ii++) {
          ...
          if ((VisionModel_B.blob_eccentricity_h[(int32_T)ii - 1] >=
              ...
              (VisionModel_B.blob_eccentricity_h[(int32_T)ii - 1] <=
               VisionModel_P.Constant1_Value_f) && (VisionModel_B.blob_extent_k
               [(int32_T)ii - 1] >= VisionModel_P.Constant2_Value_h) &&
            ...
            /* '<S48>:1:49' */
            VisionModel_B.buoy_centroids[(((int32_T)L_white - 1) << 1U)] =
              VisionModel_B.blob_centroids_m[(((int32_T)ii - 1) << 1U)];

            /* '<S48>:1:50' */
            VisionModel_B.buoy_centroids[1 + (((int32_T)L_white - 1) << 1U)] =
            ...
            /* '<S48>:1:54' */
            color_areas[(int32_T)L_white - 1] = VisionModel_B.blob_color_i
              [(int32_T)ii - 1];
          }
        }
        ...
        if (L_white == 0.0) {
          ...
          /* '<S48>:1:66' */
          minL = (VisionModel_P.Constant3_Value_i -
                  VisionModel_P.Constant2_Value_h) * 0.5;

          /* '<S48>:1:67' */
          jj = (VisionModel_P.Constant3_Value_i +
                VisionModel_P.Constant2_Value_h) * 0.5;
          ...
          for (ii = 1.0; ii <= VisionModel_B.blob_count_c; ii++) {
            ...
            if ((VisionModel_B.blob_eccentricity_h[(int32_T)ii - 1] >= r) &&
                (VisionModel_B.blob_eccentricity_h[(int32_T)ii - 1] <=
                 positive_area) && (VisionModel_B.blob_extent_k[(int32_T)ii - 1]
                 >= c) && (VisionModel_B.blob_extent_k[(int32_T)ii - 1] <= maxL))
            {
              /* '<S48>:1:77' */
              /* '<S48>:1:79' */
              L_white++;
        ...
        ii = 1.0;
        exitg = FALSE;
        while (((uint32_T)exitg == 0U) && (ii <= L_white)) {
          /* '<S46>:1:68' */
          if (color_areas[(int32_T)ii - 1] == VisionModel_U.DesiredBuoyColor) {
            /* '<S46>:1:69' */
            /* '<S46>:1:70' */
            jj = VisionModel_B.buoy_centroids[(((int32_T)ii - 1) << 1U) + 1] -
              floor(VisionModel_P.Forward_Camera_Dimensions1_Va_f[1] / 2.0);

            /* '<S46>:1:71' */
            VisionModel_B.targetZ = VisionModel_B.buoy_centroids[(((int32_T)ii -
              1) << 1U)] - floor(VisionModel_P.Forward_Camera_Dimensions1_Va_f[0]
                                 / 2.0);
        ...
        /* Embedded MATLAB Function 'torpedo/Analyze LabelMatrix for Targets': '<S50>:1' */
        /*  This function takes in vectors representing blobs present in the image, */
        /*    and returns information about how many and what kind of buoys are */
        /*    present. */
        /*  */
        ...
        /* Embedded MATLAB Function 'torpedo/Decide which target to follow': '<S51>:1' */
        ...
        if (VisionModel_U.ModeSelect == 5.0) {
          ...
          for (imgCol = 1; imgCol < 119; imgCol++) {
            for (current_color = 1; current_color < 159; current_color++) {
              ...
              VisionModel_DWork.EdgeDetection1_GV_SQUARED_DW[condition_0] = ii *
                ii;
              VisionModel_DWork.EdgeDetection1_GH_SQUARED_DW[condition_0] = jj *
                jj;
            }
          }
          ...
          for (imgCol = 1; imgCol < 119; imgCol++) {
            ...
            VisionModel_DWork.EdgeDetection1_GV_SQUARED_DW[condition_0] = minL *
              minL;
            VisionModel_DWork.EdgeDetection1_GH_SQUARED_DW[condition_0] = ii *
              ii;
            VisionModel_DWork.EdgeDetection1_GV_SQUARED_DW[current_color] = maxL
              * maxL;
            VisionModel_DWork.EdgeDetection1_GH_SQUARED_DW[current_color] = jj *
              jj;
          }
          ...
          for (current_color = 1; current_color < 159; current_color++) {
            ...
            VisionModel_DWork.EdgeDetection1_GV_SQUARED_DW[current_color] = ii *
              ii;
            VisionModel_DWork.EdgeDetection1_GH_SQUARED_DW[current_color] = minL
              * minL;
            VisionModel_DWork.EdgeDetection1_GV_SQUARED_DW[condition_0] = jj *
              jj;
              ...
              VisionModel_B.EdgeDetection1[col] =
                ((VisionModel_DWork.EdgeDetection1_GRAD_SUM_DW[col] > ii) &&
                 (((VisionModel_DWork.EdgeDetection1_GV_SQUARED_DW[col] >=
                    VisionModel_DWork.EdgeDetection1_GH_SQUARED_DW[col]) &&
                   (imgCol != 0 ?
                    VisionModel_DWork.EdgeDetection1_GRAD_SUM_DW[col - 160] <=
                    VisionModel_DWork.EdgeDetection1_GRAD_SUM_DW[col] : TRUE) &&
                   (imgCol != 119 ?
                    VisionModel_DWork.EdgeDetection1_GRAD_SUM_DW[col] >
                    VisionModel_DWork.EdgeDetection1_GRAD_SUM_DW[col + 160] :
          ...
          /* '<S43>:1:42' */
          /* '<S43>:1:44' */
          maxL = VisionModel_P.Forward_Camera_Dimensions_Val_h[0] *
          ...
          for (col = 0; col < 200; col++) {
            ...
            /* '<S43>:1:86' */
            VisionModel_B.hough_table[(i_index[col] - 1) + 399 * (j_index[col] -
              1)] = 0.0;
          }
          ...
          for (ii = 1.0; ii <= VisionModel_B.blob_count_c; ii++) {
            ...
            /* '<S43>:1:98' */
            color_areas[(int32_T)ii - 1] = VisionModel_mod(3.1415926535897931 -
              color_areas[(int32_T)ii - 1], 3.1415926535897931) -
              1.5707963267948966;
          }
          ...
          numElem = 1;
          exitg = FALSE;
          while (((uint32_T)exitg == 0U) && (numElem <= 200)) {
            /* '<S43>:1:108' */
            /* '<S43>:1:109' */
            ii = 1.0;
            exitg_0 = FALSE;
            while (((uint32_T)exitg_0 == 0U) && (ii <=
                    VisionModel_B.blob_count_c)) {
              /* '<S43>:1:109' */
              /*  Search for a match between one blob and one line */
              /* '<S43>:1:112' */
              /* '<S43>:1:113' */
              if ((theta_hold[numElem - 1] != 1000.0) && (color_areas[(int32_T)
                   ii - 1] != 1000.0)) {
                /* '<S43>:1:114' */
                c_0 = TRUE;
              } else {
              ...
              jj = VisionModel_B.blob_centroids_m[(((int32_T)ii - 1) << 1U) + 1]
                * sin(theta[numElem - 1]) + (rho[numElem - 1] * cos
                (theta[numElem - 1]) - VisionModel_B.blob_centroids_m[(((int32_T)
                ii - 1) << 1U)]);

              /* '<S43>:1:118' */
              /* '<S43>:1:119' */
              /* '<S43>:1:120' */
              if ((fabs(theta_hold[numElem - 1] - color_areas[(int32_T)ii - 1]) <=
                   0.17453292519943295) && (VisionModel_B.blob_area_p[(int32_T)
                   ii - 1] / maxL <= 0.8) && c_0 && (sqrt(rt_pow_snf(((-(1.0 /
                        tan(theta[numElem - 1]))) * jj + rho[numElem - 1] / sin
                      (theta[numElem - 1])) - VisionModel_B.blob_centroids_m
          ...
          while (((uint32_T)exitg == 0U) && (numElem <= 200)) {
            ...
            while (((uint32_T)exitg_0 == 0U) && (ii <=
              ...
              if ((fabs(theta_hold[numElem - 1] - color_areas[(int32_T)ii - 1]) <=
                ...
                /*    angles to the first */
                if (fabs(1.5707963267948966 - fabs(theta_hold[numElem - 1] -
          ...
          if (!condition) {
            ...
            /* '<S43>:1:187' */
            /* '<S43>:1:188' */
            positive_area = 1.5707963267948966 - color_areas[(int32_T)i_gate_idx
              - 1];
            ...
            /* '<S43>:1:194' */
            /* '<S43>:1:196' */
            maxL = VisionModel_B.blob_centroids_m[(((int32_T)i_gate_idx - 1) <<
              1U) + 1] * sin(positive_area) + VisionModel_B.blob_centroids_m
              [(((int32_T)i_gate_idx - 1) << 1U)] * cos(positive_area);
            ...
            r = (maxL * sin(jj) - (VisionModel_B.blob_centroids_m[(((int32_T)
                  ...
                  * cos(jj)) * sin(positive_area)) * (1.0 / (cos(positive_area) *
            ...
            maxL = (sin(color_areas[(int32_T)i_gate_idx - 1]) * ii + c) * sin
              (positive_area) + (r - cos(color_areas[(int32_T)i_gate_idx - 1]) *
                                 ii) * cos(positive_area);

            /* '<S43>:1:214' */
            /* '<S43>:1:216' */
            ii = (maxL * sin(jj) - ((cos(color_areas[(int32_T)rtb_Add1 - 1]) *
                    minL + c) * sin(jj) + (sin(color_areas[(int32_T)rtb_Add1 - 1])
                    * minL + r) * cos(jj)) * sin(positive_area)) * (1.0 / (cos
   ...
   *  Sum: '<Root>/Add2'
   */
  VisionModel_Y.MaintainHeading = ((((VisionModel_B.Constant4 + 0.0) +
    VisionModel_B.maintain_heading_n) + VisionModel_B.maintain_heading_i) +
    VisionModel_B.maintain_heading) + 0.0;
  ...
  memcpy((void *)(&VisionModel_Y.TargetSize[0]), (void *)
         (&VisionModel_B.bw_image[0]), 19200U * sizeof(real_T));
