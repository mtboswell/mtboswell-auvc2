                      ...
                      *localB, rtDW_mr_buoy_detector *localDW)
{
  ...
  for (j = 0; j < 120; j++) {
    ...
    for (num_colors = 0; num_colors < 80; num_colors++) {
      i = num_colors + 80;
      min_dist = localB->
        ColorSpaceConversion1_o1[buoy_detector_ConstP.pooled5[num_colors] * 120
        + j] * buoy_detector_ConstP.pooled2[num_colors] +
        localB->ColorSpaceConversion1_o1[buoy_detector_ConstP.pooled5[i] * 120 +
        j] * buoy_detector_ConstP.pooled2[i];
      i += 80;
      min_dist += localB->
        ColorSpaceConversion1_o1[buoy_detector_ConstP.pooled5[i] * 120 + j] *
        buoy_detector_ConstP.pooled2[i];
      i += 80;
      min_dist += localB->
        ColorSpaceConversion1_o1[buoy_detector_ConstP.pooled5[i] * 120 + j] *
        buoy_detector_ConstP.pooled2[i];
  ...
  for (num_colors = 0; num_colors < 80; num_colors++) {
    memcpy((void *)&Resize2_LineBuffer[0], (void *)(&localDW->
            Resize_IntBuffer[num_colors * 120]), 120U * sizeof(real_T));
    for (j = 0; j < 60; j++) {
      i = j + 60;
      min_dist = Resize2_LineBuffer[buoy_detector_ConstP.pooled4[j]] *
        buoy_detector_ConstP.pooled1[j] +
        Resize2_LineBuffer[buoy_detector_ConstP.pooled4[i]] *
        buoy_detector_ConstP.pooled1[i];
      i += 60;
      min_dist += Resize2_LineBuffer[buoy_detector_ConstP.pooled4[i]] *
        buoy_detector_ConstP.pooled1[i];
      i += 60;
      min_dist += Resize2_LineBuffer[buoy_detector_ConstP.pooled4[i]] *
        buoy_detector_ConstP.pooled1[i];
  ...
  for (j = 0; j < 120; j++) {
    ...
    for (num_colors = 0; num_colors < 80; num_colors++) {
      i = num_colors + 80;
      min_dist = localB->
        ColorSpaceConversion1_o2[buoy_detector_ConstP.pooled5[num_colors] * 120
        + j] * buoy_detector_ConstP.pooled2[num_colors] +
        localB->ColorSpaceConversion1_o2[buoy_detector_ConstP.pooled5[i] * 120 +
        j] * buoy_detector_ConstP.pooled2[i];
      i += 80;
      min_dist += localB->
        ColorSpaceConversion1_o2[buoy_detector_ConstP.pooled5[i] * 120 + j] *
        buoy_detector_ConstP.pooled2[i];
      i += 80;
      min_dist += localB->
        ColorSpaceConversion1_o2[buoy_detector_ConstP.pooled5[i] * 120 + j] *
  ...
  for (num_colors = 0; num_colors < 80; num_colors++) {
    memcpy((void *)&Resize2_LineBuffer[0], (void *)(&localDW->
            Resize1_IntBuffer[num_colors * 120]), 120U * sizeof(real_T));
    for (j = 0; j < 60; j++) {
      i = j + 60;
      min_dist = Resize2_LineBuffer[buoy_detector_ConstP.pooled4[j]] *
        buoy_detector_ConstP.pooled1[j] +
  ...
  for (j = 0; j < 120; j++) {
    ...
    for (num_colors = 0; num_colors < 80; num_colors++) {
      i = num_colors + 80;
      min_dist = localB->
        ColorSpaceConversion1_o3[buoy_detector_ConstP.pooled5[num_colors] * 120
        + j] * buoy_detector_ConstP.pooled2[num_colors] +
        localB->ColorSpaceConversion1_o3[buoy_detector_ConstP.pooled5[i] * 120 +
        j] * buoy_detector_ConstP.pooled2[i];
      i += 80;
      min_dist += localB->
        ColorSpaceConversion1_o3[buoy_detector_ConstP.pooled5[i] * 120 + j] *
        buoy_detector_ConstP.pooled2[i];
      i += 80;
      min_dist += localB->
        ColorSpaceConversion1_o3[buoy_detector_ConstP.pooled5[i] * 120 + j] *
  ...
  for (num_colors = 0; num_colors < 80; num_colors++) {
    memcpy((void *)&Resize2_LineBuffer[0], (void *)(&localDW->
            Resize2_IntBuffer[num_colors * 120]), 120U * sizeof(real_T));
    for (j = 0; j < 60; j++) {
      i = j + 60;
      min_dist = Resize2_LineBuffer[buoy_detector_ConstP.pooled4[j]] *
        buoy_detector_ConstP.pooled1[j] +
    ...
    for (j = 0; j < 160; j++) {
      ...
      for (k = 1.0; k <= localB->num_colors; k++) {
        ...
        dist = sqrt((rt_pow_snf(localB->ref_colors[(int32_T)k - 1] -
          localB->ColorSpaceConversion1_o1[120 * j + i], 2.0) + rt_pow_snf
                     (localB->ref_colors[(int32_T)k + 49] -
                      localB->ColorSpaceConversion1_o2[120 * j + i], 2.0)) +
                    rt_pow_snf(localB->ref_colors[(int32_T)k + 99] -
          localB->ColorSpaceConversion1_o3[120 * j + i], 2.0));
        if (dist < min_dist) {
          /* '<S9>:1:18' */
          /* '<S9>:1:19' */
          min_dist = dist;
  ...
  for (k = 1.0; k <= min_dist; k++) {
    ...
    for (i = 0; i < 120; i++) {
      ...
      for (j = 0; j < 160; j++) {
        /* '<S13>:1:49' */
        if (localB->LabelMatrix[120 * j + i] != k) {
          /* '<S13>:1:50' */
          /* '<S13>:1:51' */
          localB->LogicMatrix[i + 120 * j] = FALSE;
        } else {
          /* '<S13>:1:53' */
          localB->LogicMatrix[i + 120 * j] = TRUE;
        }
      }
    }
    ...
    if (tmp_0 < 50) {
      ...
      for (b_i = 1U; b_i <= TNumBlobs; b_i = (uint8_T)((uint32_T)b_i + 1U)) {
        ...
        localB->blob_hues[tmp_0 - 1] = localB->ref_colors[(int32_T)
          localB->LabelMatrix[((int32_T)dist - 1) * 120 + ((int32_T)b - 1)] - 1];
      }
