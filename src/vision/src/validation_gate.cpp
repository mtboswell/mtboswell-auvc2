  ...
  *localDW)
{
  ...
  for (m = 0; m < 160; m++) {
    ...
    for (iy = 0; iy < 80; iy++) {
      ...
      rtb_num_colors = localB->
        ColorSpaceConversion1_o1[validation_gate_ConstP.pooled5[iy] * 160 + m] *
        validation_gate_ConstP.pooled3[iy] + localB->
        ColorSpaceConversion1_o1[validation_gate_ConstP.pooled5[imgIdxLL] * 160
        + m] * validation_gate_ConstP.pooled3[imgIdxLL];
      imgIdxLL += 80;
      rtb_num_colors += localB->
        ColorSpaceConversion1_o1[validation_gate_ConstP.pooled5[imgIdxLL] * 160
        + m] * validation_gate_ConstP.pooled3[imgIdxLL];
  ...
  for (iy = 0; iy < 80; iy++) {
    ...
    for (m = 0; m < 60; m++) {
      ...
      for (imgIdxUR = 0; imgIdxUR < 6; imgIdxUR++) {
        rtb_num_colors +=
          Resize2_LineBuffer[validation_gate_ConstP.pooled4[imgIdxLL]] *
          validation_gate_ConstP.pooled2[imgIdxLL];
        imgIdxLL += 60;
      }
  ...
  for (m = 0; m < 160; m++) {
    ...
    for (iy = 0; iy < 80; iy++) {
      imgIdxLL = iy + 80;
      rtb_num_colors = localB->
        ColorSpaceConversion1_o2[validation_gate_ConstP.pooled5[iy] * 160 + m] *
        validation_gate_ConstP.pooled3[iy] + localB->
        ColorSpaceConversion1_o2[validation_gate_ConstP.pooled5[imgIdxLL] * 160
        + m] * validation_gate_ConstP.pooled3[imgIdxLL];
      imgIdxLL += 80;
      rtb_num_colors += localB->
        ColorSpaceConversion1_o2[validation_gate_ConstP.pooled5[imgIdxLL] * 160
  ...
  for (m = 0; m < 160; m++) {
    ...
    for (iy = 0; iy < 80; iy++) {
      imgIdxLL = iy + 80;
      rtb_num_colors = localB->
        ColorSpaceConversion1_o3[validation_gate_ConstP.pooled5[iy] * 160 + m] *
        validation_gate_ConstP.pooled3[iy] + localB->
        ColorSpaceConversion1_o3[validation_gate_ConstP.pooled5[imgIdxLL] * 160
        + m] * validation_gate_ConstP.pooled3[imgIdxLL];
      imgIdxLL += 80;
      rtb_num_colors += localB->
        ColorSpaceConversion1_o3[validation_gate_ConstP.pooled5[imgIdxLL] * 160
  ...
  for (j = 1; j < 119; j++) {
    ...
    for (m = 0; m < 6; m++) {
      rtb_num_colors += localB->LabelMatrix[flag + localDW->
        EdgeDetection_HOU_DW[m]] * validation_gate_ConstP.EdgeDetection_HC_RTP[m];
      ...
      accumFour += localB->LabelMatrix[imgIdxUR + localDW->
        EdgeDetection_VOD_DW[m]] * validation_gate_ConstP.EdgeDetection_VC_RTP[m];
    }
  ...
  for (imgIdxUR = 1; imgIdxUR < 159; imgIdxUR++) {
    ...
    for (m = 0; m < 6; m++) {
      ...
      accumThree += localB->LabelMatrix[imgIdxUR + localDW->
        EdgeDetection_HOL_DW[m]] * validation_gate_ConstP.EdgeDetection_HC_RTP[m];
  ...
  for (m = 0; m < 6; m++) {
    rtb_num_colors += localB->LabelMatrix[19040 + localDW->
      EdgeDetection_VOUR_DW[m]] * validation_gate_ConstP.EdgeDetection_VC_RTP[m];
