 ...
 * Real-Time Workshop file generated on : Mon Jun 27 18:23:04 2011
 * TLC version                          : 7.6 (Jul 13 2010)
 * C/C++ source code generated on       : Mon Jun 27 18:23:05 2011
 *
  ...
  rtDW_mr_validation_gate *localDW)
{
  ...
  for (imgIdxUR = 0; imgIdxUR < 19200; imgIdxUR++) {
    ...
    if (localB->sf_LowPassFilter1.Iout[imgIdxUR] <= 0.039286085583733095) {
      accumTwo = localB->sf_LowPassFilter1.Iout[imgIdxUR] / 12.923054468333255;
    } else {
      accumTwo = rt_pow_snf((localB->sf_LowPassFilter1.Iout[imgIdxUR] +
        0.055000519817226347) / 1.0550005198172263, 2.4);
    }
  ...
  for (imgIdxUR = 0; imgIdxUR < 19200; imgIdxUR++) {
    ...
    localB->ColorSpaceConversion1_o3[imgIdxUR] = (accumTwo - accumOne) * 200.0;
  }
  ...
  for (m = 0; m < 160; m++) {
    ...
    for (j = 0; j < 80; j++) {
      i = j + 80;
      accumOne = localB->
        ColorSpaceConversion1_o1[validation_gate_ConstP.pooled7[j] * 160 + m] *
        validation_gate_ConstP.pooled4[j] + localB->
        ColorSpaceConversion1_o1[validation_gate_ConstP.pooled7[i] * 160 + m] *
  ...
  for (j = 0; j < 80; j++) {
    ...
    for (m = 0; m < 60; m++) {
      ...
      for (imgCol = 0; imgCol < 6; imgCol++) {
        accumOne += Resize2_LineBuffer[validation_gate_ConstP.pooled6[i]] *
  ...
  for (m = 0; m < 160; m++) {
    ...
    for (j = 0; j < 80; j++) {
      i = j + 80;
      accumOne = localB->
        ColorSpaceConversion1_o2[validation_gate_ConstP.pooled7[j] * 160 + m] *
        validation_gate_ConstP.pooled4[j] + localB->
        ColorSpaceConversion1_o2[validation_gate_ConstP.pooled7[i] * 160 + m] *
        validation_gate_ConstP.pooled4[i];
      i += 80;
      accumOne += localB->
        ColorSpaceConversion1_o2[validation_gate_ConstP.pooled7[i] * 160 + m] *
  ...
  for (m = 0; m < 160; m++) {
    ...
    for (j = 0; j < 80; j++) {
      i = j + 80;
      accumOne = localB->
        ColorSpaceConversion1_o3[validation_gate_ConstP.pooled7[j] * 160 + m] *
        validation_gate_ConstP.pooled4[j] + localB->
        ColorSpaceConversion1_o3[validation_gate_ConstP.pooled7[i] * 160 + m] *
        validation_gate_ConstP.pooled4[i];
      i += 80;
      accumOne += localB->
        ColorSpaceConversion1_o3[validation_gate_ConstP.pooled7[i] * 160 + m] *
  ...
  for (imgIdxUR = 0; imgIdxUR < 160; imgIdxUR++) {
    ...
    for (j = 0; j < 120; j++) {
      ...
      /* '<S12>:1:23' */
      rty_LabelMatrix[imgIdxUR + 160 * j] = accumTwo;
    }
  }
  ...
  for (j = 0; j < 5; j++) {
    ...
    for (accumOne = 1.0; accumOne <= (*rty_num_colors); accumOne++) {
      /* '<S5>:1:18' */
      if (ref_colors[(int32_T)accumOne - 1] > maxL[j]) {
        /* '<S5>:1:19' */
        /* '<S5>:1:20' */
        gate_i[j] = accumOne;

        /* '<S5>:1:21' */
        maxL[j] = ref_colors[(int32_T)accumOne - 1];
      }
    }
  ...
  for (flag = 0; flag < 160; flag++) {
    ...
    for (j = 0; j < 120; j++) {
      /* '<S5>:1:32' */
      /* '<S5>:1:34' */
      for (imgIdxUR = 0; imgIdxUR < 5; imgIdxUR++) {
        /* '<S5>:1:34' */
        /*  Allow only the brightest max_num_cols colors */
        if ((rty_LabelMatrix[160 * j + flag] == gate_i[imgIdxUR]) && (fabs
             (ref_colors[(int32_T)rty_LabelMatrix[160 * j + flag] + 99] + 2.7) <=
             50.0)) {
          /* '<S5>:1:37' */
  ...
  for (imgCol = 1; imgCol < 119; imgCol++) {
    for (j = 1; j < 159; j++) {
      ...
      for (m = 0; m < 6; m++) {
        ...
        accumTwo += rty_bw_image[flag + localDW->EdgeDetection_HO_DW[m]] *
  ...
  for (imgCol = 1; imgCol < 119; imgCol++) {
    ...
    for (m = 0; m < 6; m++) {
      accumOne += rty_bw_image[flag + localDW->EdgeDetection_HOU_DW[m]] *
        validation_gate_ConstP.EdgeDetection_HC_RTP[m];
      accumTwo += rty_bw_image[j + localDW->EdgeDetection_HOD_DW[m]] *
        validation_gate_ConstP.EdgeDetection_HC_RTP[m];
    ...
    localDW->EdgeDetection_GH_SQUARED_DW[j] = accumTwo * accumTwo;
  }
  ...
  for (j = 1; j < 159; j++) {
    ...
    for (m = 0; m < 6; m++) {
      accumOne += rty_bw_image[j + localDW->EdgeDetection_VOL_DW[m]] *
        validation_gate_ConstP.EdgeDetection_VC_RTP[m];
      accumTwo += rty_bw_image[flag + localDW->EdgeDetection_VOR_DW[m]] *
        validation_gate_ConstP.EdgeDetection_VC_RTP[m];
  ...
  for (m = 0; m < 6; m++) {
    accumOne += rty_bw_image[localDW->EdgeDetection_VOUL_DW[m]] *
      validation_gate_ConstP.EdgeDetection_VC_RTP[m];
    accumTwo += rty_bw_image[localDW->EdgeDetection_HOUL_DW[m]] *
      validation_gate_ConstP.EdgeDetection_HC_RTP[m];
  ...
  for (m = 0; m < 6; m++) {
    accumOne += rty_bw_image[19040 + localDW->EdgeDetection_VOUR_DW[m]] *
      validation_gate_ConstP.EdgeDetection_VC_RTP[m];
    accumTwo += rty_bw_image[19040 + localDW->EdgeDetection_HOUR_DW[m]] *
      validation_gate_ConstP.EdgeDetection_HC_RTP[m];
  ...
  for (flag = 0; flag < 6; flag++) {
    ...
    if (vbinv[flag] == 0.0) {
      ...
      /* '<S2>:1:156' */
      accumTwo = tavgv[flag] / vbinv[flag];
    }
  ...
  /* '<S2>:1:237' */
  accumOne = opphmoppiecjmglf_norm(u);
  u[0] = 1.0 / accumOne;
  u[1] = u[1] / accumOne;

  /*  Compute target location */
  /* '<S2>:1:240' */
  accumOne = sqrt(rt_pow_snf(accumFour - accumTwo, 2.0) + rt_pow_snf(c -
    accumThree, 2.0)) * 0.282;
  ...
  if (aaieaaiejmopnglf_sum(rho_gate) > 0.0) {
    ...
    /* '<S2>:1:256' */
    (*rty_TargetZ) = ((accumTwo + accumFour) / 2.0 + accumOne * u[0]) - 80.0;

    /* '<S2>:1:257' */
    (*rty_TargetY) = ((accumThree + c) / 2.0 + accumOne * u[1]) - 60.0;
  }
