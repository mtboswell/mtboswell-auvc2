  ...
  rtDW_mr_validation_gate *localDW)
{
  ...
  for (imgIdxUR = 0; imgIdxUR < 19200; imgIdxUR++) {
    ...
    if (localB->sf_LowPassFilter.Iout[imgIdxUR] <= 0.039286085583733095) {
      cnt = localB->sf_LowPassFilter.Iout[imgIdxUR] / 12.923054468333255;
    } else {
      cnt = rt_pow_snf((localB->sf_LowPassFilter.Iout[imgIdxUR] +
                        0.055000519817226347) / 1.0550005198172263, 2.4);
    }

    if (localB->sf_LowPassFilter1.Iout[imgIdxUR] <= 0.039286085583733095) {
      scale = localB->sf_LowPassFilter1.Iout[imgIdxUR] / 12.923054468333255;
    } else {
      scale = rt_pow_snf((localB->sf_LowPassFilter1.Iout[imgIdxUR] +
                          0.055000519817226347) / 1.0550005198172263, 2.4);
    }

    if (localB->sf_LowPassFilter2.Iout[imgIdxUR] <= 0.039286085583733095) {
      accumThree = localB->sf_LowPassFilter2.Iout[imgIdxUR] / 12.923054468333255;
    } else {
      accumThree = rt_pow_snf((localB->sf_LowPassFilter2.Iout[imgIdxUR] +
        0.055000519817226347) / 1.0550005198172263, 2.4);
    }
    ...
    accumFour = (0.41239079926596 * cnt + 0.35758433938388 * scale) +
      0.18048078840183 * accumThree;
    c = (0.21263900587151 * cnt + 0.71516867876776 * scale) + 0.07219231536073 *
      accumThree;
    cnt = (0.01933081871559 * cnt + 0.11919477979463 * scale) + 0.95053215224966
      * accumThree;
    ...
    localB->ColorSpaceConversion1_o3[imgIdxUR] = cnt;
  }

  /* Convert from XYZ to L*a*b* */
  for (imgIdxUR = 0; imgIdxUR < 19200; imgIdxUR++) {
    cnt = localB->ColorSpaceConversion1_o1[imgIdxUR] / 0.9641986557609;
    ...
    if (cnt > 0.0088564516790356311) {
      accumFour = rt_pow_snf(cnt, 0.33333333333333331);
    } else {
      accumFour = 7.7870370370370372 * cnt + 0.13793103448275862;
    }

    if (localB->ColorSpaceConversion1_o2[imgIdxUR] > 0.0088564516790356311) {
      scale = rt_pow_snf(localB->ColorSpaceConversion1_o2[imgIdxUR],
                         0.33333333333333331);
    } else {
      scale = 7.7870370370370372 * localB->ColorSpaceConversion1_o2[imgIdxUR] +
        0.13793103448275862;
    }

    if (accumThree > 0.0088564516790356311) {
      cnt = rt_pow_snf(accumThree, 0.33333333333333331);
    } else {
      cnt = 7.7870370370370372 * accumThree + 0.13793103448275862;
    }

    /* assign the results */
    localB->ColorSpaceConversion1_o1[imgIdxUR] = 116.0 * scale - 16.0;
    localB->ColorSpaceConversion1_o2[imgIdxUR] = (accumFour - scale) * 500.0;
    localB->ColorSpaceConversion1_o3[imgIdxUR] = (scale - cnt) * 200.0;
  }
  ...
  for (m = 0; m < 160; m++) {
    ...
    for (j = 0; j < 80; j++) {
      imgIdxUR = j + 80;
      cnt = localB->ColorSpaceConversion1_o1[validation_gate_ConstP.pooled7[j] *
        160 + m] * validation_gate_ConstP.pooled4[j] +
        localB->ColorSpaceConversion1_o1[validation_gate_ConstP.pooled7[imgIdxUR]
        * 160 + m] * validation_gate_ConstP.pooled4[imgIdxUR];
      imgIdxUR += 80;
      cnt += localB->
        ColorSpaceConversion1_o1[validation_gate_ConstP.pooled7[imgIdxUR] * 160
        + m] * validation_gate_ConstP.pooled4[imgIdxUR];
      localDW->Resize_IntBuffer[i] = cnt;
      i += 160;
    }
  }
  ...
  for (j = 0; j < 80; j++) {
    ...
    for (m = 0; m < 60; m++) {
      ...
      cnt = 0.0;
      for (i = 0; i < 6; i++) {
        cnt += Resize2_LineBuffer[validation_gate_ConstP.pooled6[imgIdxUR]] *
          validation_gate_ConstP.pooled3[imgIdxUR];
        imgIdxUR += 60;
      }

      localB->Resize[imgCol] = cnt;
      imgCol++;
    }
  }
  ...
  for (m = 0; m < 160; m++) {
    ...
    for (j = 0; j < 80; j++) {
      imgIdxUR = j + 80;
      cnt = localB->ColorSpaceConversion1_o2[validation_gate_ConstP.pooled7[j] *
        160 + m] * validation_gate_ConstP.pooled4[j] +
        localB->ColorSpaceConversion1_o2[validation_gate_ConstP.pooled7[imgIdxUR]
        * 160 + m] * validation_gate_ConstP.pooled4[imgIdxUR];
      imgIdxUR += 80;
      cnt += localB->
        ColorSpaceConversion1_o2[validation_gate_ConstP.pooled7[imgIdxUR] * 160
        + m] * validation_gate_ConstP.pooled4[imgIdxUR];
      localDW->Resize1_IntBuffer[i] = cnt;
      i += 160;
    }
  }
  ...
  for (m = 0; m < 160; m++) {
    ...
    for (j = 0; j < 80; j++) {
      imgIdxUR = j + 80;
      cnt = localB->ColorSpaceConversion1_o3[validation_gate_ConstP.pooled7[j] *
        160 + m] * validation_gate_ConstP.pooled4[j] +
        localB->ColorSpaceConversion1_o3[validation_gate_ConstP.pooled7[imgIdxUR]
        * 160 + m] * validation_gate_ConstP.pooled4[imgIdxUR];
      imgIdxUR += 80;
      cnt += localB->
        ColorSpaceConversion1_o3[validation_gate_ConstP.pooled7[imgIdxUR] * 160
        + m] * validation_gate_ConstP.pooled4[imgIdxUR];
      localDW->Resize2_IntBuffer[i] = cnt;
      i += 160;
    }
  }
    ...
    while ((imgIdxUR <= 60) && (flag == 0)) {
      ...
      while ((j <= 80) && (flag == 0)) {
        ...
        while ((uint32_T)accumThree <= (uint32_T)imgCol) {
          /* '<S13>:1:34' */
          /* '<S13>:1:35' */
          accumFour = sqrt((rt_pow_snf(rty_ref_colors[(int32_T)accumThree + 49]
            - localB->Resize1[(j - 1) * 60 + (imgIdxUR - 1)], 2.0) * 1.5 +
                            rt_pow_snf(rty_ref_colors[(int32_T)accumThree - 1] -
            localB->Resize[(j - 1) * 60 + (imgIdxUR - 1)], 2.0)) + rt_pow_snf
                           (rty_ref_colors[(int32_T)accumThree + 99] -
        ...
        if ((cnt > 30.0) && (imgCol < 50)) {
          ...
          /* '<S13>:1:52' */
          localB->LabelMatrix[(imgIdxUR - 1) + 60 * (j - 1)] = (uint32_T)scale;
        }
  ...
  /* '<S13>:1:80' */
  for (cnt = 1.0; (uint32_T)cnt <= (uint32_T)imgCol; cnt++) {
    /* '<S13>:1:80' */
    /* '<S13>:1:81' */
    rty_ref_colors[(int32_T)cnt - 1] = ref_color_L_avg[(int32_T)cnt - 1];

    /* '<S13>:1:82' */
    rty_ref_colors[(int32_T)cnt + 49] = ref_color_a_avg[(int32_T)cnt - 1];

    /* '<S13>:1:83' */
    rty_ref_colors[(int32_T)cnt + 99] = ref_color_b_avg[(int32_T)cnt - 1];
  }
  ...
  for (imgIdxUR = 0; imgIdxUR < 160; imgIdxUR++) {
    ...
    for (j = 0; j < 120; j++) {
      ...
      /* '<S12>:1:16' */
      for (accumThree = 1.0; accumThree <= (*rty_num_colors); accumThree++) {
        /* '<S12>:1:16' */
        /* '<S12>:1:17' */
        accumFour = sqrt((rt_pow_snf(rty_ref_colors[(int32_T)accumThree - 1] -
          localB->ColorSpaceConversion1_o1[160 * j + imgIdxUR], 2.0) +
                          rt_pow_snf(rty_ref_colors[(int32_T)accumThree + 49] -
          localB->ColorSpaceConversion1_o2[160 * j + imgIdxUR], 2.0)) +
                         rt_pow_snf(rty_ref_colors[(int32_T)accumThree + 99] -
      ...
      /* '<S12>:1:23' */
      rty_LabelMatrix[imgIdxUR + 160 * j] = scale;
    }
  }

  /* '<S12>:1:27' */

  /* Embedded MATLAB: '<S1>/Identify Validation Gate2' incorporates:
   *  Constant: '<Root>/Dist_Thresh1'
   *  Constant: '<Root>/Dist_Thresh2'
   *  Constant: '<Root>/Dist_Thresh3'
   *  Constant: '<Root>/Dist_Thresh4'
   */
  ...
  /*  Initialize */
  /*  Used to help eliminate background noise */
  /* '<S5>:1:11' */
  /* '<S5>:1:12' */
  for (i = 0; i < 5; i++) {
    maxL[i] = 0.0;
  ...
  for (j = 0; j < 5; j++) {
    /* '<S5>:1:16' */
    /* '<S5>:1:18' */
    for (cnt = 1.0; cnt <= (*rty_num_colors); cnt++) {
      /* '<S5>:1:18' */
      if (ref_colors[(int32_T)cnt - 1] > maxL[j]) {
        /* '<S5>:1:19' */
        /* '<S5>:1:20' */
        gate_i[j] = cnt;

        /* '<S5>:1:21' */
        maxL[j] = ref_colors[(int32_T)cnt - 1];
      }
    }

    /* '<S5>:1:25' */
    ref_colors[(int32_T)gate_i[j] - 1] = -1.0;
  }
  ...
  /*     for c = 1:cols */
  /*          */
  /*         for n = 1:max_num_cols */
  /*              */
  /*             % Allow only the brightest max_num_cols colors */
  /*             if( LabelMatrix(r,c) == gate_i(n) ) */
  /*                  */
  /*                 % Check a, b values to ensure only white colors pass */
  /*                 % through */
  /*                 if( abs(ref_colors(LabelMatrix(r,c), 3) + 2.7) <= eps_b ) */
  /*                     intensity_image(r,c) = 1; */
  /*                 end */
  /*                  */
  /*             end */
  /*              */
  ...
  /* end */
  /* } */
  /*  Pay attention only to lightness */
  /* '<S5>:1:56' */
  cnt = ref_colors[0];
  flag = 1;
  j = 1;
  guard = FALSE;
  if (rtIsNaN(ref_colors[0])) {
    searchingForNonNaN = TRUE;
    i = 2;
    exitg = FALSE;
    while (((uint32_T)exitg == 0U) && (i < 51)) {
      j++;
      if (!rtIsNaN(ref_colors[j - 1])) {
        cnt = ref_colors[j - 1];
        flag = i;
  ...
  if (guard) {
    for (flag++; flag < 51; flag++) {
      j++;
      if (ref_colors[j - 1] > cnt) {
        cnt = ref_colors[j - 1];
      }
    }
  }
  ...
  for (flag = 0; flag < 160; flag++) {
    ...
    for (j = 0; j < 120; j++) {
      /* '<S5>:1:59' */
      if ((ref_colors[(int32_T)rty_LabelMatrix[160 * j + flag] + 49] >= 20.0) &&
          (ref_colors[(int32_T)rty_LabelMatrix[160 * j + flag] + 49] <= 60.0) &&
          (ref_colors[(int32_T)rty_LabelMatrix[160 * j + flag] + 99] >= 32.0) &&
          (ref_colors[(int32_T)rty_LabelMatrix[160 * j + flag] + 99] <= 42.0)) {
        /* '<S5>:1:60' */
        /* '<S5>:1:61' */
        localB->intensity_image[flag + 160 * j] = 1.0;
      } else {
        /* '<S5>:1:63' */
        localB->intensity_image[flag + 160 * j] = ref_colors[(int32_T)
          rty_LabelMatrix[160 * j + flag] - 1] / cnt;
      }
    }
  }

  /* S-Function (svipgraythresh): '<Root>/Autothreshold' */
  memset((void *)&Autothreshold_P_DW[0], 0, (uint32_T)((int32_T)
          MWDSP_SIZEOF_DOUBLE << 8U));
  memset((void *)&Autothreshold_MU_DW[0], 0, (uint32_T)((int32_T)
          MWDSP_SIZEOF_DOUBLE << 8U));
  for (imgIdxUR = 0; imgIdxUR < 19200; imgIdxUR++) {
    if (localB->intensity_image[imgIdxUR] < 0.0) {
      Autothreshold_P_DW[0] = Autothreshold_P_DW[0] + 1.0;
    } else if (localB->intensity_image[imgIdxUR] > 1.0) {
      Autothreshold_P_DW[255] = Autothreshold_P_DW[255] + 1.0;
    } else {
      Autothreshold_P_DW[(uint8_T)(localB->intensity_image[imgIdxUR] * 255.0 +
        0.5)] = Autothreshold_P_DW[(uint8_T)(localB->intensity_image[imgIdxUR] *
        255.0 + 0.5)] + 1.0;
    }
  }

  for (imgIdxUR = 0; imgIdxUR < 256; imgIdxUR++) {
    Autothreshold_P_DW[imgIdxUR] = Autothreshold_P_DW[imgIdxUR] / 19200.0;
  }

  Autothreshold_MU_DW[0] = Autothreshold_P_DW[0];
  cnt = 2.0;
  for (imgIdxUR = 0; imgIdxUR < 255; imgIdxUR++) {
    Autothreshold_MU_DW[imgIdxUR + 1] = Autothreshold_P_DW[imgIdxUR + 1] * cnt +
      Autothreshold_MU_DW[imgIdxUR];
    cnt++;
  }

  for (imgIdxUR = 0; imgIdxUR < 255; imgIdxUR++) {
    Autothreshold_P_DW[imgIdxUR + 1] = Autothreshold_P_DW[imgIdxUR + 1] +
      Autothreshold_P_DW[imgIdxUR];
  }

  for (imgIdxUR = 0; imgIdxUR < 256; imgIdxUR++) {
    cnt = Autothreshold_P_DW[imgIdxUR] * Autothreshold_MU_DW[255] -
      Autothreshold_MU_DW[imgIdxUR];
    cnt *= cnt;
    Autothreshold_P_DW[imgIdxUR] = cnt / ((1.0 - Autothreshold_P_DW[imgIdxUR]) *
      Autothreshold_P_DW[imgIdxUR]);
  }
  ...
  cnt = 0.0;
  for (imgIdxUR = 0; imgIdxUR < 256; imgIdxUR++) {
    if (Autothreshold_P_DW[imgIdxUR] > cnt) {
      cnt = Autothreshold_P_DW[imgIdxUR];
      flag = imgIdxUR;
    }
  }

  cnt = (real_T)flag / 255.0;
  for (imgIdxUR = 0; imgIdxUR < 19200; imgIdxUR++) {
    localB->Autothreshold[imgIdxUR] = (localB->intensity_image[imgIdxUR] > cnt);
  }
  ...
  for (flag = 0; flag < 160; flag++) {
    ...
    for (j = 0; j < 120; j++) {
      /* '<S3>:1:14' */
      /* '<S3>:1:15' */
      localB->intensity_image[flag + 160 * j] = rty_ref_colors[(int32_T)
        rty_LabelMatrix[160 * j + flag] - 1];
  ...
  for (imgIdxUR = 0; imgIdxUR < 19200; imgIdxUR++) {
    c = (localB->intensity_image[imgIdxUR] + 16.0) / 116.0;
    accumFour = localB->a[imgIdxUR] / 500.0 + c;
    cnt = localB->b[imgIdxUR] / -200.0 + c;
    ...
    if (cnt <= 0.20689655172413793) {
      cnt -= 0.13793103448275862;
      cnt /= 7.7870370370370372;
    } else {
      cnt = rt_pow_snf(cnt, 3.0);
    }

    accumFour *= 0.9641986557609;
    cnt *= 0.82511648322104;

    /* assign the results */
    localB->ColorSpaceConversion1_o1_l[imgIdxUR] = accumFour;
    localB->ColorSpaceConversion1_o2_d[imgIdxUR] = c;
    localB->ColorSpaceConversion1_o3_k[imgIdxUR] = cnt;
  }
  ...
  for (imgIdxUR = 0; imgIdxUR < 19200; imgIdxUR++) {
    ...
    cnt = (0.055630079697 * localB->ColorSpaceConversion1_o1_l[imgIdxUR] +
    ...
    if (cnt <= 0.00304) {
      cnt *= 12.923054468333255;
    } else {
      cnt = 1.0550005198172263 * rt_pow_snf(cnt, 0.41666666666666669) -
        0.055000519817226347;
    }
  ...
  for (i = 0; i < 19200; i++) {
    rty_bw_image[i] = (real_T)localB->Autothreshold[i];
  }
  ...
  for (imgCol = 1; imgCol < 119; imgCol++) {
    for (j = 1; j < 159; j++) {
      ...
      flag = imgCol * 160 + j;
      for (m = 0; m < 6; m++) {
        cnt += rty_bw_image[flag + localDW->EdgeDetection_VO_DW[m]] *
          validation_gate_ConstP.EdgeDetection_VC_RTP[m];
        scale += rty_bw_image[flag + localDW->EdgeDetection_HO_DW[m]] *
          validation_gate_ConstP.EdgeDetection_HC_RTP[m];
      }

      localDW->EdgeDetection_GV_SQUARED_DW[flag] = cnt * cnt;
      localDW->EdgeDetection_GH_SQUARED_DW[flag] = scale * scale;
    }
  }
  ...
  for (imgCol = 1; imgCol < 119; imgCol++) {
    ...
    j = imgCol * 160 + 159;
    for (m = 0; m < 6; m++) {
      cnt += rty_bw_image[flag + localDW->EdgeDetection_HOU_DW[m]] *
        validation_gate_ConstP.EdgeDetection_HC_RTP[m];
      scale += rty_bw_image[j + localDW->EdgeDetection_HOD_DW[m]] *
        validation_gate_ConstP.EdgeDetection_HC_RTP[m];
      accumThree += rty_bw_image[flag + localDW->EdgeDetection_VOU_DW[m]] *
        validation_gate_ConstP.EdgeDetection_VC_RTP[m];
    ...
    localDW->EdgeDetection_GH_SQUARED_DW[flag] = cnt * cnt;
    localDW->EdgeDetection_GV_SQUARED_DW[j] = accumFour * accumFour;
    localDW->EdgeDetection_GH_SQUARED_DW[j] = scale * scale;
  }
  ...
  for (j = 1; j < 159; j++) {
    ...
    flag = 19040 + j;
    for (m = 0; m < 6; m++) {
      cnt += rty_bw_image[j + localDW->EdgeDetection_VOL_DW[m]] *
        validation_gate_ConstP.EdgeDetection_VC_RTP[m];
      scale += rty_bw_image[flag + localDW->EdgeDetection_VOR_DW[m]] *
        validation_gate_ConstP.EdgeDetection_VC_RTP[m];
      accumThree += rty_bw_image[j + localDW->EdgeDetection_HOL_DW[m]] *
        validation_gate_ConstP.EdgeDetection_HC_RTP[m];
      accumFour += rty_bw_image[flag + localDW->EdgeDetection_HOR_DW[m]] *
        validation_gate_ConstP.EdgeDetection_HC_RTP[m];
    }

    localDW->EdgeDetection_GV_SQUARED_DW[j] = cnt * cnt;
    localDW->EdgeDetection_GH_SQUARED_DW[j] = accumThree * accumThree;
    localDW->EdgeDetection_GV_SQUARED_DW[flag] = scale * scale;
  ...
  accumFour = 0.0;
  for (m = 0; m < 6; m++) {
    cnt += rty_bw_image[localDW->EdgeDetection_VOUL_DW[m]] *
      validation_gate_ConstP.EdgeDetection_VC_RTP[m];
    scale += rty_bw_image[localDW->EdgeDetection_HOUL_DW[m]] *
      validation_gate_ConstP.EdgeDetection_HC_RTP[m];
    accumThree += rty_bw_image[159 + localDW->EdgeDetection_VOLL_DW[m]] *
      validation_gate_ConstP.EdgeDetection_VC_RTP[m];
    accumFour += rty_bw_image[159 + localDW->EdgeDetection_HOLL_DW[m]] *
      validation_gate_ConstP.EdgeDetection_HC_RTP[m];
  }

  localDW->EdgeDetection_GV_SQUARED_DW[0] = cnt * cnt;
  localDW->EdgeDetection_GH_SQUARED_DW[0] = scale * scale;
  ...
  accumFour = 0.0;
  for (m = 0; m < 6; m++) {
    cnt += rty_bw_image[19040 + localDW->EdgeDetection_VOUR_DW[m]] *
      validation_gate_ConstP.EdgeDetection_VC_RTP[m];
    scale += rty_bw_image[19040 + localDW->EdgeDetection_HOUR_DW[m]] *
      validation_gate_ConstP.EdgeDetection_HC_RTP[m];
    accumThree += rty_bw_image[19199 + localDW->EdgeDetection_VOLR_DW[m]] *
      validation_gate_ConstP.EdgeDetection_VC_RTP[m];
    accumFour += rty_bw_image[19199 + localDW->EdgeDetection_HOLR_DW[m]] *
      validation_gate_ConstP.EdgeDetection_HC_RTP[m];
  }

  localDW->EdgeDetection_GV_SQUARED_DW[19040] = cnt * cnt;
  localDW->EdgeDetection_GH_SQUARED_DW[19040] = scale * scale;
  ...
  for (m = 0; m < 19200; m++) {
    ...
    scale += localDW->EdgeDetection_GRAD_SUM_DW[m] *
  ...
  for (imgCol = 0; imgCol < 120; imgCol++) {
    for (j = 0; j < 160; j++) {
      m = imgCol * 160 + j;
      rty_edge_image[m] = ((localDW->EdgeDetection_GRAD_SUM_DW[m] > cnt) &&
  ...
  for (m = 0; m < 200; m++) {
    ...
    for (imgIdxUR = 0; imgIdxUR < 399; imgIdxUR++) {
      ...
      for (j = 0; j < 180; j++) {
        /* '<S2>:1:70' */
        if (rty_theta[399 * j + imgIdxUR] > cnt) {
          /* '<S2>:1:71' */
          /* '<S2>:1:72' */
  ...
  for (j = 0; j < 200; j++) {
    ...
    /*    tolerance */
    if ((fabs(theta[j] - 1.5707963267948966) <= 0.17453292519943295) || (fabs
         (theta[j] + 1.5707963267948966) <= 0.17453292519943295)) {
      /* '<S2>:1:110' */
      exitg = TRUE;
    } else {
      /* '<S2>:1:110' */
    ...
    /*  Vertical line */
    guard = FALSE;
    if (fabs(theta[j]) <= 0.17453292519943295) {
      /* '<S2>:1:113' */
      if ((rho[j] == 0.0) && (theta[j] == 0.0)) {
        /* '<S2>:1:113' */
        searchingForNonNaN = TRUE;
      } else {
        /* '<S2>:1:113' */
      ...
      if (!searchingForNonNaN) {
        ...
        while (((uint32_T)searchingForNonNaN == 0U) && (flag <= 6)) {
          ...
          if (fabs(rho[j] - (real_T)rbinv[flag - 1]) <= 16.0) {
            /* '<S2>:1:118' */
            /* '<S2>:1:119' */
            ravgv[flag - 1] = ravgv[flag - 1] + rho[j];

            /* '<S2>:1:120' */
            tavgv[flag - 1] = tavgv[flag - 1] + theta[j];
    ...
    if (guard && exitg) {
      ...
      if (theta[j] < 0.0) {
        /* '<S2>:1:129' */
        /* '<S2>:1:130' */
        theta[j] = theta[j] + 3.1415926535897931;
      ...
      while (((uint32_T)exitg == 0U) && (flag <= 6)) {
        /* '<S2>:1:134' */
        /*  Place each rho value into a bin */
        if (fabs(rho[j] - (real_T)rbinh[flag - 1]) <= 16.0) {
          /* '<S2>:1:137' */
          /* '<S2>:1:138' */
          ravgh[flag - 1] = ravgh[flag - 1] + rho[j];

          /* '<S2>:1:139' */
          tavgh[flag - 1] = tavgh[flag - 1] + theta[j];
  ...
  for (flag = 0; flag < 6; flag++) {
    ...
    if (vbinv[flag] == 0.0) {
      ...
      /* '<S2>:1:155' */
      cnt = ravgv[flag] / vbinv[flag];

      /* '<S2>:1:156' */
      scale = tavgv[flag] / vbinv[flag];
    }
    ...
    if (vbinh[flag] == 0.0) {
      ...
      /* '<S2>:1:164' */
      accumThree = ravgh[flag] / vbinh[flag];

      /* '<S2>:1:165' */
      accumFour = tavgh[flag] / vbinh[flag];
    }

    ravgv[flag] = cnt;
    tavgv[flag] = scale;
    ravgh[flag] = accumThree;
    tavgh[flag] = accumFour;
  }
  ...
  for (flag = 0; flag < 6; flag++) {
    ...
    if ((ravgv[flag] < cnt) && (ravgv[flag] != 0.0)) {
      /* '<S2>:1:176' */
      /* '<S2>:1:177' */
      cnt = ravgv[flag];
    ...
    if ((ravgv[flag] > scale) && (ravgv[flag] != 0.0)) {
      /* '<S2>:1:183' */
      /* '<S2>:1:184' */
      scale = ravgv[flag];
  ...
  for (flag = 0; flag < 6; flag++) {
    ...
    if ((vbinh[flag] > cnt) && (ravgh[flag] != 0.0)) {
      /* '<S2>:1:205' */
      /* '<S2>:1:206' */
      cnt = vbinh[flag];
  ...
  /* '<S2>:1:222' */
  scale = (rho_gate_idx_1 / cos(theta_gate_idx_1) - rho_gate_idx_0 / cos
           (theta_gate_idx_0)) * (1.0 / (tan(theta_gate_idx_1) - tan
    (theta_gate_idx_0)));

  /* '<S2>:1:223' */
  accumThree = (rho_gate_idx_1 - scale * sin(theta_gate_idx_1)) * (1.0 / cos
    (theta_gate_idx_1));
  ...
  /* '<S2>:1:237' */
  cnt = opphmoppiecjmglf_norm(u);
  u[0] = 1.0 / cnt;
  u[1] = u[1] / cnt;

  /*  Compute target location */
  /* '<S2>:1:240' */
  cnt = sqrt(rt_pow_snf(accumFour - scale, 2.0) + rt_pow_snf(c - accumThree, 2.0))
    * 0.282;
  ...
  if (aaieaaiejmopnglf_sum(rho_gate) > 0.0) {
    ...
    /* '<S2>:1:256' */
    (*rty_TargetZ) = ((scale + accumFour) / 2.0 + cnt * u[0]) - 80.0;

    /* '<S2>:1:257' */
    (*rty_TargetY) = ((accumThree + c) / 2.0 + cnt * u[1]) - 60.0;
  }
  ...
  for (i = 0; i < 19200; i++) {
    rty_R[i] = rty_bw_image[i];
    rty_G[i] = rty_bw_image[i];
    rty_B[i] = rty_bw_image[i];
  }

  /* Draw all X marks. */
  for (imgIdxLR = 0; imgIdxLR < 2; imgIdxLR += 2) {
    cnt = rtb_VectorConcatenate[imgIdxLR];
    if ((rtb_VectorConcatenate[imgIdxLR] < 4.503599627370496E+15) &&
        (rtb_VectorConcatenate[imgIdxLR] > -4.503599627370496E+15)) {
      cnt = floor(rtb_VectorConcatenate[imgIdxLR] + 0.5);
    }

    m = (int32_T)cnt;
    cnt = rtb_VectorConcatenate[imgIdxLR + 1];
    if ((rtb_VectorConcatenate[imgIdxLR + 1] < 4.503599627370496E+15) &&
        (rtb_VectorConcatenate[imgIdxLR + 1] > -4.503599627370496E+15)) {
      cnt = floor(rtb_VectorConcatenate[imgIdxLR + 1] + 0.5);
    }
    ...
    while (!done) {
      ...
      if (line_idx_3 < 0) {
        imgIdxUR = (int32_T)((uint32_T)imgIdxUR | 1U);
      } else {
        if (line_idx_3 > 119) {
          imgIdxUR = (int32_T)((uint32_T)imgIdxUR | 2U);
        }
      }

      if (!((uint32_T)i | (uint32_T)imgIdxUR)) {
        /* Line falls completely within bounds. */
        done = TRUE;
        searchingForNonNaN = TRUE;
      } else if ((uint32_T)i & (uint32_T)imgIdxUR) {
        ...
        if ((uint32_T)i & 4U) {
          /* Violated RMin. */
          imgCol = (0 - line_idx_6) * j;
          if (((imgCol >= 0) && (flag >= 0)) || ((imgCol < 0) && (flag < 0))) {
            line_idx_5 += (div_s32_floor(imgCol << 1U, flag) + 1) >> 1;
          } else {
            line_idx_5 += -((div_s32_floor((-imgCol) << 1U, flag) + 1) >> 1);
          }
          ...
          /* Violated CMin. */
          imgCol = (0 - line_idx_5) * flag;
          if (((imgCol >= 0) && (j >= 0)) || ((imgCol < 0) && (j < 0))) {
            line_idx_6 += (div_s32_floor(imgCol << 1U, j) + 1) >> 1;
          } else {
            line_idx_6 += -((div_s32_floor((-imgCol) << 1U, j) + 1) >> 1);
          }
        ...
        flag = line_idx_4 - line_idx_6;
        j = line_idx_3 - line_idx_5;
        if ((uint32_T)imgIdxUR & 4U) {
          /* Violated RMin. */
          imgCol = (0 - line_idx_4) * j;
          if (((imgCol >= 0) && (flag >= 0)) || ((imgCol < 0) && (flag < 0))) {
            line_idx_3 += (div_s32_floor(imgCol << 1U, flag) + 1) >> 1;
          } else {
            line_idx_3 += -((div_s32_floor((-imgCol) << 1U, flag) + 1) >> 1);
          }
          ...
          line_idx_4 = 0;
          guard = TRUE;
        } else if ((uint32_T)imgIdxUR & 8U) {
          /* Violated RMax. */
          imgCol = (159 - line_idx_4) * j;
          if (((imgCol >= 0) && (flag >= 0)) || ((imgCol < 0) && (flag < 0))) {
            line_idx_3 += (div_s32_floor(imgCol << 1U, flag) + 1) >> 1;
          } else {
            line_idx_3 += -((div_s32_floor((-imgCol) << 1U, flag) + 1) >> 1);
          ...
          line_idx_4 = 159;
          guard = TRUE;
        } else if ((uint32_T)imgIdxUR & 1U) {
          /* Violated CMin. */
          imgCol = (0 - line_idx_3) * flag;
          if (((imgCol >= 0) && (j >= 0)) || ((imgCol < 0) && (j < 0))) {
            line_idx_4 += (div_s32_floor(imgCol << 1U, j) + 1) >> 1;
          } else {
            line_idx_4 += -((div_s32_floor((-imgCol) << 1U, j) + 1) >> 1);
          }
    ...
    if (searchingForNonNaN) {
      ...
      for (imgCol = line_idx_6; imgCol <= line_idx_4; imgCol++) {
        rty_R[j] = 1.0;
        rty_G[j] = 0.0;
        rty_B[j] = 0.0;
        j += 161;
