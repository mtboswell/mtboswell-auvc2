 ...
 * Real-Time Workshop file generated on : Tue Jun 21 02:40:28 2011
 * TLC version                          : 7.6 (Jul 13 2010)
 * C/C++ source code generated on       : Tue Jun 21 02:40:28 2011
 *
  ...
  *localDW)
{
  ...
  for (flag = 0; flag < 6; flag++) {
    ...
    if (vbinv[flag] == 0.0) {
      ...
      /* '<S2>:1:151' */
      rtb_num_colors = ravgv[flag] / vbinv[flag];

      /* '<S2>:1:152' */
      accumFour = tavgv[flag] / vbinv[flag];
    }
    ...
    if (vbinh[flag] == 0.0) {
      ...
      /* '<S2>:1:160' */
      accumOne = ravgh[flag] / vbinh[flag];

      /* '<S2>:1:161' */
      accumTwo = tavgh[flag] / vbinh[flag];
    }

    ravgv[flag] = rtb_num_colors;
    tavgv[flag] = accumFour;
    ravgh[flag] = accumOne;
    tavgh[flag] = accumTwo;
  }
  ...
  /* '<S2>:1:218' */
  accumOne = (rho_gate_idx_1 / cos(theta_gate_idx_1) - rho_gate_idx_0 / cos
              (theta_gate_idx_0)) * (1.0 / (tan(theta_gate_idx_1) - tan
    (theta_gate_idx_0)));

  /* '<S2>:1:219' */
  /* '<S2>:1:221' */
  accumTwo = (rho_gate_idx_0 / cos(theta_gate_idx_0) - rho_gate_idx / cos
  ...
  /* '<S2>:1:233' */
  accumThree = opphmoppiecjmglf_norm(maxL);
  maxL[0] = 1.0 / accumThree;
  maxL[1] = maxL[1] / accumThree;
  ...
  if (aaieaaiejmopnglf_sum(rho_gate) > 0.0) {
    ...
    /* '<S2>:1:252' */
    (*rty_TargetZ) = (sqrt(rt_pow_snf((rho_gate_idx_0 - accumTwo * sin
      (theta_gate_idx_0)) * (1.0 / cos(theta_gate_idx_0)) - (rho_gate_idx_1 -
      accumOne * sin(theta_gate_idx_1)) * (1.0 / cos(theta_gate_idx_1)), 2.0) +
      rt_pow_snf(accumTwo - accumOne, 2.0)) * 0.282 * maxL[0] + (accumOne +
      accumTwo) / 2.0) - 320.0;
  }

  /* '<S2>:1:259' */
  (*rty_TargetY) = rho[0] * 180.0 / 3.1415926535897931;
