  ...
  rtDW_mr_validation_gate *localDW)
{
  ...
  for (i = 0; i < 19200; i++) {
    ...
    if (localB->sf_LowPassFilter.Iout[i] <= 0.039286085583733095) {
      rmin = localB->sf_LowPassFilter.Iout[i] / 12.923054468333255;
    } else {
      rmin = rt_pow_snf((localB->sf_LowPassFilter.Iout[i] + 0.055000519817226347)
                        / 1.0550005198172263, 2.4);
    }

    if (localB->sf_LowPassFilter1.Iout[i] <= 0.039286085583733095) {
      gate_i_idx = localB->sf_LowPassFilter1.Iout[i] / 12.923054468333255;
    } else {
      gate_i_idx = rt_pow_snf((localB->sf_LowPassFilter1.Iout[i] +
        0.055000519817226347) / 1.0550005198172263, 2.4);
    }

    if (localB->sf_LowPassFilter2.Iout[i] <= 0.039286085583733095) {
      maxL_idx = localB->sf_LowPassFilter2.Iout[i] / 12.923054468333255;
    } else {
      maxL_idx = rt_pow_snf((localB->sf_LowPassFilter2.Iout[i] +
        0.055000519817226347) / 1.0550005198172263, 2.4);
    }
    ...
    maxL_idx_0 = (0.41239079926596 * rmin + 0.35758433938388 * gate_i_idx) +
      0.18048078840183 * maxL_idx;
    gate_i_idx_0 = (0.21263900587151 * rmin + 0.71516867876776 * gate_i_idx) +
      0.07219231536073 * maxL_idx;
    rmin = (0.01933081871559 * rmin + 0.11919477979463 * gate_i_idx) +
      0.95053215224966 * maxL_idx;
    ...
    /* assign the results */
    localB->ColorSpaceConversion1_o1[i] = maxL_idx_0;
    localB->ColorSpaceConversion1_o2[i] = gate_i_idx_0;
    localB->ColorSpaceConversion1_o3[i] = rmin;
  }

  /* Convert from XYZ to L*a*b* */
  for (i = 0; i < 19200; i++) {
    rmin = localB->ColorSpaceConversion1_o1[i] / 0.9641986557609;
    maxL_idx = localB->ColorSpaceConversion1_o3[i] / 0.82511648322104;

    /* Prepare Xf, Yf, and Zf for computation of a* and b* components */
    if (rmin > 0.0088564516790356311) {
      maxL_idx_0 = rt_pow_snf(rmin, 0.33333333333333331);
    } else {
      maxL_idx_0 = 7.7870370370370372 * rmin + 0.13793103448275862;
    }

    if (localB->ColorSpaceConversion1_o2[i] > 0.0088564516790356311) {
      gate_i_idx = rt_pow_snf(localB->ColorSpaceConversion1_o2[i],
        0.33333333333333331);
    } else {
      gate_i_idx = 7.7870370370370372 * localB->ColorSpaceConversion1_o2[i] +
        0.13793103448275862;
    }

    if (maxL_idx > 0.0088564516790356311) {
      rmin = rt_pow_snf(maxL_idx, 0.33333333333333331);
    } else {
      rmin = 7.7870370370370372 * maxL_idx + 0.13793103448275862;
    }

    /* assign the results */
    localB->ColorSpaceConversion1_o1[i] = 116.0 * gate_i_idx - 16.0;
    localB->ColorSpaceConversion1_o2[i] = (maxL_idx_0 - gate_i_idx) * 500.0;
    localB->ColorSpaceConversion1_o3[i] = (gate_i_idx - rmin) * 200.0;
  }
  ...
  for (m = 0; m < 160; m++) {
    ...
    for (j = 0; j < 80; j++) {
      OutCode = j + 80;
      rmin = localB->ColorSpaceConversion1_o1[validation_gate_ConstP.pooled8[j] *
        160 + m] * validation_gate_ConstP.pooled4[j] +
        localB->ColorSpaceConversion1_o1[validation_gate_ConstP.pooled8[OutCode]
        * 160 + m] * validation_gate_ConstP.pooled4[OutCode];
      OutCode += 80;
      rmin += localB->
        ColorSpaceConversion1_o1[validation_gate_ConstP.pooled8[OutCode] * 160 +
        m] * validation_gate_ConstP.pooled4[OutCode];
      localDW->Resize_IntBuffer[idxROI] = rmin;
      idxROI += 160;
    }
  }
  ...
  for (j = 0; j < 80; j++) {
    ...
    for (m = 0; m < 60; m++) {
      OutCode = m;
      rmin = 0.0;
      for (num_colors = 0; num_colors < 6; num_colors++) {
        rmin += Resize2_LineBuffer[validation_gate_ConstP.pooled7[OutCode]] *
          validation_gate_ConstP.pooled3[OutCode];
        OutCode += 60;
      }

      localB->Resize[i] = rmin;
      i++;
    }
  }
  ...
  for (m = 0; m < 160; m++) {
    ...
    for (j = 0; j < 80; j++) {
      OutCode = j + 80;
      rmin = localB->ColorSpaceConversion1_o2[validation_gate_ConstP.pooled8[j] *
        160 + m] * validation_gate_ConstP.pooled4[j] +
        localB->ColorSpaceConversion1_o2[validation_gate_ConstP.pooled8[OutCode]
        * 160 + m] * validation_gate_ConstP.pooled4[OutCode];
      OutCode += 80;
      rmin += localB->
        ColorSpaceConversion1_o2[validation_gate_ConstP.pooled8[OutCode] * 160 +
        m] * validation_gate_ConstP.pooled4[OutCode];
      localDW->Resize1_IntBuffer[idxROI] = rmin;
      idxROI += 160;
    }
  }
  ...
  for (m = 0; m < 160; m++) {
    ...
    for (j = 0; j < 80; j++) {
      OutCode = j + 80;
      rmin = localB->ColorSpaceConversion1_o3[validation_gate_ConstP.pooled8[j] *
        160 + m] * validation_gate_ConstP.pooled4[j] +
        localB->ColorSpaceConversion1_o3[validation_gate_ConstP.pooled8[OutCode]
        * 160 + m] * validation_gate_ConstP.pooled4[OutCode];
      OutCode += 80;
      rmin += localB->
        ColorSpaceConversion1_o3[validation_gate_ConstP.pooled8[OutCode] * 160 +
        m] * validation_gate_ConstP.pooled4[OutCode];
      localDW->Resize2_IntBuffer[idxROI] = rmin;
      idxROI += 160;
    }
  }
  ...
  while (i <= 60) {
    ...
    while ((i <= 60) && (flag == 0)) {
      ...
      while ((j <= 80) && (flag == 0)) {
        ...
        /* '<S13>:1:33' */
        gate_i_idx = 1.0;
        while ((uint32_T)maxL_idx <= (uint32_T)num_colors) {
          /* '<S13>:1:34' */
          /* '<S13>:1:35' */
          maxL_idx_0 = sqrt((rt_pow_snf(rty_ref_colors[(int32_T)maxL_idx + 49] -
            localB->Resize1[(j - 1) * 60 + (i - 1)], 2.0) * 1.5 + rt_pow_snf
                             (rty_ref_colors[(int32_T)maxL_idx - 1] -
                              localB->Resize[(j - 1) * 60 + (i - 1)], 2.0)) +
                            rt_pow_snf(rty_ref_colors[(int32_T)maxL_idx + 99] -
            localB->Resize2[(j - 1) * 60 + (i - 1)], 2.0) * 1.5);
          if (rmin > maxL_idx_0) {
            /* '<S13>:1:36' */
            /* '<S13>:1:37' */
            rmin = maxL_idx_0;
        ...
        if ((rmin > 20.0) && (num_colors < 50)) {
          /* '<S13>:1:45' */
          /* '<S13>:1:46' */
          rty_ref_colors[num_colors] = localB->Resize[(j - 1) * 60 + (i - 1)];
          rty_ref_colors[num_colors + 50] = localB->Resize1[(j - 1) * 60 + (i -
            1)];
          rty_ref_colors[num_colors + 100] = localB->Resize2[(j - 1) * 60 + (i -
            1)];
          ...
          /* '<S13>:1:52' */
          localB->LabelMatrix[(i - 1) + 60 * (j - 1)] = (uint32_T)gate_i_idx;
        }
  ...
  for (i = 0; i < 60; i++) {
    ...
    for (flag = 0; flag < 80; flag++) {
      ...
      /* '<S13>:1:69' */
      /* '<S13>:1:70' */
      ref_color_L_avg[(int32_T)localB->LabelMatrix[i + 60 * flag] - 1] =
        ref_color_L_avg[(int32_T)localB->LabelMatrix[60 * flag + i] - 1] +
        localB->Resize[60 * flag + i];

      /* '<S13>:1:71' */
      ref_color_a_avg[(int32_T)localB->LabelMatrix[i + 60 * flag] - 1] =
        ref_color_a_avg[(int32_T)localB->LabelMatrix[60 * flag + i] - 1] +
        localB->Resize1[60 * flag + i];

      /* '<S13>:1:72' */
      ref_color_b_avg[(int32_T)localB->LabelMatrix[i + 60 * flag] - 1] =
        ref_color_b_avg[(int32_T)localB->LabelMatrix[60 * flag + i] - 1] +
        localB->Resize2[60 * flag + i];

      /* '<S13>:1:73' */
      color_count[(int32_T)localB->LabelMatrix[i + 60 * flag] - 1] =
        color_count[(int32_T)localB->LabelMatrix[60 * flag + i] - 1] + 1.0;
    }
  }
  ...
  /* '<S13>:1:80' */
  for (rmin = 1.0; (uint32_T)rmin <= (uint32_T)num_colors; rmin++) {
    /* '<S13>:1:80' */
    /* '<S13>:1:81' */
    rty_ref_colors[(int32_T)rmin - 1] = ref_color_L_avg[(int32_T)rmin - 1];

    /* '<S13>:1:82' */
    rty_ref_colors[(int32_T)rmin + 49] = ref_color_a_avg[(int32_T)rmin - 1];

    /* '<S13>:1:83' */
    rty_ref_colors[(int32_T)rmin + 99] = ref_color_b_avg[(int32_T)rmin - 1];
  }

  (*rty_num_colors) = (real_T)num_colors;
  ...
  for (i = 0; i < 160; i++) {
    ...
    for (j = 0; j < 120; j++) {
      ...
      /* '<S12>:1:16' */
      for (maxL_idx = 1.0; maxL_idx <= (*rty_num_colors); maxL_idx++) {
        /* '<S12>:1:16' */
        /* '<S12>:1:17' */
        maxL_idx_0 = sqrt((rt_pow_snf(rty_ref_colors[(int32_T)maxL_idx - 1] -
          localB->ColorSpaceConversion1_o1[160 * j + i], 2.0) + rt_pow_snf
                           (rty_ref_colors[(int32_T)maxL_idx + 49] -
      ...
      /* '<S12>:1:23' */
      rty_LabelMatrix[i + 160 * j] = gate_i_idx;
    }
  }
  ...
  for (i = 0; i < 19200; i++) {
    gate_i_idx_0 = (localB->L[i] + 16.0) / 116.0;
    maxL_idx_0 = localB->a[i] / 500.0 + gate_i_idx_0;
    rmin = localB->b[i] / -200.0 + gate_i_idx_0;
    if (maxL_idx_0 <= 0.20689655172413793) {
      maxL_idx_0 -= 0.13793103448275862;
      maxL_idx_0 /= 7.7870370370370372;
    } else {
      maxL_idx_0 = rt_pow_snf(maxL_idx_0, 3.0);
    }

    if (gate_i_idx_0 <= 0.20689655172413793) {
      gate_i_idx_0 -= 0.13793103448275862;
      gate_i_idx_0 /= 7.7870370370370372;
    } else {
      gate_i_idx_0 = rt_pow_snf(gate_i_idx_0, 3.0);
    }

    if (rmin <= 0.20689655172413793) {
      rmin -= 0.13793103448275862;
      rmin /= 7.7870370370370372;
    } else {
      rmin = rt_pow_snf(rmin, 3.0);
    }

    maxL_idx_0 *= 0.9641986557609;
    rmin *= 0.82511648322104;

    /* assign the results */
    localB->ColorSpaceConversion1_o1_l[i] = maxL_idx_0;
    localB->ColorSpaceConversion1_o2_d[i] = gate_i_idx_0;
    localB->ColorSpaceConversion1_o3_k[i] = rmin;
  }
  ...
  for (i = 0; i < 19200; i++) {
    ...
    /* BT.709 reference primaries for sRGB and CIE standard illuminant */
    /* D65, 2 degree observer */
    maxL_idx_0 = (3.24096994190451 * localB->ColorSpaceConversion1_o1_l[i] +
                  -1.53738317757009 * localB->ColorSpaceConversion1_o2_d[i]) +
      -0.49861076029299 * localB->ColorSpaceConversion1_o3_k[i];
    gate_i_idx_0 = (-0.96924363628087 * localB->ColorSpaceConversion1_o1_l[i] +
                    1.87596750150771 * localB->ColorSpaceConversion1_o2_d[i]) +
      0.04155505740718 * localB->ColorSpaceConversion1_o3_k[i];
    rmin = (0.055630079697 * localB->ColorSpaceConversion1_o1_l[i] +
            -0.20397695888899 * localB->ColorSpaceConversion1_o2_d[i]) +
      1.05697151424288 * localB->ColorSpaceConversion1_o3_k[i];

    /* Apply gamma correction to get R'G'B' */
    if (maxL_idx_0 <= 0.00304) {
      maxL_idx_0 *= 12.923054468333255;
    } else {
      maxL_idx_0 = 1.0550005198172263 * rt_pow_snf(maxL_idx_0,
        0.41666666666666669) - 0.055000519817226347;
    }

    if (gate_i_idx_0 <= 0.00304) {
      gate_i_idx_0 *= 12.923054468333255;
    } else {
      gate_i_idx_0 = 1.0550005198172263 * rt_pow_snf(gate_i_idx_0,
        0.41666666666666669) - 0.055000519817226347;
    }

    if (rmin <= 0.00304) {
      rmin *= 12.923054468333255;
    } else {
      rmin = 1.0550005198172263 * rt_pow_snf(rmin, 0.41666666666666669) -
  ...
  /* '<S5>:1:22' */
  /* '<S5>:1:24' */
  for (rmin = 1.0; rmin <= (*rty_num_colors); rmin++) {
    /* '<S5>:1:24' */
    if (ref_colors[(int32_T)rmin - 1] > maxL_idx_0) {
      /* '<S5>:1:25' */
      /* '<S5>:1:26' */
      gate_i_idx_0 = rmin;

      /* '<S5>:1:27' */
      maxL_idx_0 = ref_colors[(int32_T)rmin - 1];
    }
  }
  ...
  for (rmin = 1.0; rmin <= (*rty_num_colors); rmin++) {
    /* '<S5>:1:24' */
    if (ref_colors[(int32_T)rmin - 1] > maxL_idx) {
      /* '<S5>:1:25' */
      /* '<S5>:1:26' */
      gate_i_idx = rmin;

      /* '<S5>:1:27' */
      maxL_idx = ref_colors[(int32_T)rmin - 1];
    }
  }
  ...
  for (i = 0; i < 19200; i++) {
    rty_edge_image[i] = (rty_bw_image[i] != 0.0);
  }
  ...
  for (i = 0; i < 200; i++) {
    ...
    /*    tolerance */
    if ((fabs(theta[i] - 1.5707963267948966) <= 0.17453292519943295) || (fabs
         (theta[i] + 1.5707963267948966) <= 0.17453292519943295)) {
      /* '<S2>:1:110' */
      visited = TRUE;
    } else {
      /* '<S2>:1:110' */
    ...
    /*  Vertical line */
    visited_0 = FALSE;
    if (fabs(theta[i]) <= 0.17453292519943295) {
      /* '<S2>:1:113' */
      if ((rho[i] == 0.0) && (theta[i] == 0.0)) {
        /* '<S2>:1:113' */
        isInBound = TRUE;
      } else {
        /* '<S2>:1:113' */
      ...
      if (!isInBound) {
        ...
        while (((uint32_T)isInBound == 0U) && (flag <= 6)) {
          ...
          if (fabs(rho[i] - (real_T)rbinv[flag - 1]) <= 16.0) {
            /* '<S2>:1:118' */
            /* '<S2>:1:119' */
            ravgv[flag - 1] = ravgv[flag - 1] + rho[i];

            /* '<S2>:1:120' */
            tavgv[flag - 1] = tavgv[flag - 1] + theta[i];
    ...
    if (visited_0 && visited) {
      ...
      if (theta[i] < 0.0) {
        /* '<S2>:1:129' */
        /* '<S2>:1:130' */
        theta[i] = theta[i] + 3.1415926535897931;
      ...
      while (((uint32_T)isInBound == 0U) && (flag <= 6)) {
        /* '<S2>:1:134' */
        /*  Place each rho value into a bin */
        if (fabs(rho[i] - (real_T)rbinh[flag - 1]) <= 16.0) {
          /* '<S2>:1:137' */
          /* '<S2>:1:138' */
          ravgh[flag - 1] = ravgh[flag - 1] + rho[i];

          /* '<S2>:1:139' */
          tavgh[flag - 1] = tavgh[flag - 1] + theta[i];
  ...
  for (flag = 0; flag < 6; flag++) {
    ...
    if (vbinv[flag] == 0.0) {
      ...
      /* '<S2>:1:155' */
      maxL_idx_0 = ravgv[flag] / vbinv[flag];

      /* '<S2>:1:156' */
      gate_i_idx_0 = tavgv[flag] / vbinv[flag];
    }
    ...
    if (vbinh[flag] == 0.0) {
      ...
      /* '<S2>:1:164' */
      maxL_idx = ravgh[flag] / vbinh[flag];

      /* '<S2>:1:165' */
      gate_i_idx = tavgh[flag] / vbinh[flag];
    }

    ravgv[flag] = maxL_idx_0;
    tavgv[flag] = gate_i_idx_0;
    ravgh[flag] = maxL_idx;
    tavgh[flag] = gate_i_idx;
  }
  ...
  for (flag = 0; flag < 6; flag++) {
    ...
    if ((ravgv[flag] < rmin) && (ravgv[flag] != 0.0)) {
      /* '<S2>:1:176' */
      /* '<S2>:1:177' */
      rmin = ravgv[flag];
    ...
    if ((ravgv[flag] > gate_i_idx) && (ravgv[flag] != 0.0)) {
      /* '<S2>:1:183' */
      /* '<S2>:1:184' */
      gate_i_idx = ravgv[flag];
  ...
  for (flag = 0; flag < 6; flag++) {
    ...
    if ((vbinh[flag] > rmin) && (ravgh[flag] != 0.0)) {
      /* '<S2>:1:205' */
      /* '<S2>:1:206' */
      rmin = vbinh[flag];
  ...
  /* '<S2>:1:263' */
  /* '<S2>:1:265' */
  (*rty_TargetY) = 180.0 * theta[0] / 3.1415926535897931;
  (*rty_TargetZ) = 180.0 * theta[1] / 3.1415926535897931;
  ...
  for (idxROI = 0; idxROI < 2; idxROI += 2) {
    maxL_idx_0 = rtb_VectorConcatenate[idxROI];
    if ((rtb_VectorConcatenate[idxROI] < 4.503599627370496E+15) &&
        (rtb_VectorConcatenate[idxROI] > -4.503599627370496E+15)) {
      maxL_idx_0 = floor(rtb_VectorConcatenate[idxROI] + 0.5);
    }

    m = (int32_T)maxL_idx_0;
    maxL_idx_0 = rtb_VectorConcatenate[idxROI + 1];
    if ((rtb_VectorConcatenate[idxROI + 1] < 4.503599627370496E+15) &&
        (rtb_VectorConcatenate[idxROI + 1] > -4.503599627370496E+15)) {
      maxL_idx_0 = floor(rtb_VectorConcatenate[idxROI + 1] + 0.5);
    }

    col = (int32_T)maxL_idx_0;
    line_idx_2 = m - 7;
    line_idx_1 = col - 7;
    line_idx_0 = m + 7;
    line_idx = col + 7;
    ...
    while (!done) {
      ...
      if (line_idx_5 < 0) {
        num_colors = (int32_T)((uint32_T)num_colors | 1U);
      } else {
        if (line_idx_5 > 119) {
          num_colors = (int32_T)((uint32_T)num_colors | 2U);
        }
      }
      ...
      if (!((uint32_T)num_colors | (uint32_T)OutCode)) {
        /* Line falls completely within bounds. */
        done = TRUE;
        isInBound = TRUE;
      } else if ((uint32_T)num_colors & (uint32_T)OutCode) {
        /* Line falls completely out of bounds. */
        done = TRUE;
        isInBound = FALSE;
      } else if ((uint32_T)num_colors != 0U) {
        ...
        if ((uint32_T)num_colors & 4U) {
          /* Violated RMin. */
          i = (0 - line_idx_6) * j;
          if (((i >= 0) && (flag >= 0)) || ((i < 0) && (flag < 0))) {
            line_idx_5 += (div_s32_floor(i << 1U, flag) + 1) >> 1;
          } else {
            line_idx_5 += -((div_s32_floor((-i) << 1U, flag) + 1) >> 1);
          }
          ...
          line_idx_6 = 0;
          visited = TRUE;
        } else if ((uint32_T)num_colors & 8U) {
          /* Violated RMax. */
          i = (159 - line_idx_6) * j;
          if (((i >= 0) && (flag >= 0)) || ((i < 0) && (flag < 0))) {
            line_idx_5 += (div_s32_floor(i << 1U, flag) + 1) >> 1;
          ...
          line_idx_6 = 159;
          visited = TRUE;
        } else if ((uint32_T)num_colors & 1U) {
          /* Violated CMin. */
          i = (0 - line_idx_5) * flag;
          if (((i >= 0) && (j >= 0)) || ((i < 0) && (j < 0))) {
            line_idx_6 += (div_s32_floor(i << 1U, j) + 1) >> 1;
          } else {
            line_idx_6 += -((div_s32_floor((-i) << 1U, j) + 1) >> 1);
          }
        ...
        if ((uint32_T)OutCode & 4U) {
          ...
          i = (0 - line_idx_4) * j;
          if (((i >= 0) && (flag >= 0)) || ((i < 0) && (flag < 0))) {
            line_idx_3 += (div_s32_floor(i << 1U, flag) + 1) >> 1;
          } else {
            line_idx_3 += -((div_s32_floor((-i) << 1U, flag) + 1) >> 1);
          }
          ...
          i = (0 - line_idx_3) * flag;
          if (((i >= 0) && (j >= 0)) || ((i < 0) && (j < 0))) {
            line_idx_4 += (div_s32_floor(i << 1U, j) + 1) >> 1;
          } else {
            line_idx_4 += -((div_s32_floor((-i) << 1U, j) + 1) >> 1);
          }
    ...
    if (isInBound) {
      ...
      for (i = line_idx_6; i <= line_idx_4; i++) {
        rty_R[j] = 0.0;
        rty_G[j] = 0.0;
        rty_B[j] = 0.0;
        j += 161;
