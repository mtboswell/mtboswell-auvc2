...
static void torpedo_Outputs(const boolean_T U0[307200], real_T Y0[100], real_T
 ...
 *    '<S4>/Low Pass Filter2'
 */
void torpedo_LowPassFilter(const real_T rtu_I[307200], rtB_LowPassFilter_torpedo
  *localB)
{
  real_T G[9];
  real_T Gsum;
  ...
  Y1[50], real_T Y2[50], uint8_T *Y3, rtDW_mr_torpedo *localDW)
{
  ...
  memset((void *)(&localDW->PAD_DW[0]), (int32_T)0U, 643U * sizeof(uint8_T));
  currentLabel = 1U;
  loop = 0;
  idx = 643;
  for (n = 0; n < 480; n++) {
    for (maxc = 0; maxc < 640; maxc++) {
  ...
  memset((void *)(&localDW->PAD_DW[idx]), (int32_T)0U, 641U * sizeof(uint8_T));
  loop = 1;
  stackIdx = 0U;
  pixIdx = 0U;
  for (n = 0; n < 480; n++) {
          ...
          for (pixListMinc = 0; pixListMinc < 8; pixListMinc++) {
            ...
            if (localDW->PAD_DW[walkerIdx] == 255) {
              ...
              localDW->N_PIXLIST_DW[pixIdx] = (int16_T)((int16_T)(walkerIdx /
                642U) - 1);
              localDW->M_PIXLIST_DW[pixIdx] = (int16_T)(walkerIdx % 642U - 1U);
              pixIdx++;
...
{
  int32_T i;
  static int16_T tmp[8] = { -1, 641, 642, 643, 1, -641, -642, -643 };
...
void mr_torpedo(const real_T rtu_R_forward_in[307200], const real_T
                rtu_G_forward_in[307200], const real_T rtu_B_forward_in[307200],
                ...
                rtB_mr_torpedo *localB, rtDW_mr_torpedo *localDW)
{
  ...
  for (i = 0; i < 307200; i++) {
    ...
    dist = (0.41239079926596 * min_dist + 0.35758433938388 * index) +
      0.18048078840183 * b;
    k_0 = (0.21263900587151 * min_dist + 0.71516867876776 * index) +
      0.07219231536073 * b;
    ...
    localB->ColorSpaceConversion1_o2[i] = k_0;
  ...
  for (i = 0; i < 307200; i++) {
    ...
    if (min_dist > 0.0088564516790356311) {
      k_0 = rt_pow_snf(min_dist, 0.33333333333333331);
    } else {
      k_0 = 7.7870370370370372 * min_dist + 0.13793103448275862;
    }
    ...
    localB->ColorSpaceConversion1_o2[i] = (k_0 - index) * 500.0;
  ...
  for (j = 0; j < 640; j++) {
    ...
    for (num_colors = 0; num_colors < 80; num_colors++) {
      ...
      min_dist = 0.0;
      for (k = 0; k < 12; k++) {
        min_dist += localB->ColorSpaceConversion1_o1[torpedo_ConstP.pooled4[i] *
          640 + j] * torpedo_ConstP.pooled2[i];
        i += 80;
      }

      localDW->Resize_IntBuffer[indx] = min_dist;
      indx += 640;
    }
  }
  ...
  for (num_colors = 0; num_colors < 80; num_colors++) {
    memcpy((void *)(&localDW->Resize2_LineBuffer[0]), (void *)
           (&localDW->Resize_IntBuffer[num_colors * 640]), 640U * sizeof(real_T));
    for (j = 0; j < 60; j++) {
      i = j;
      min_dist = 0.0;
      for (k = 0; k < 22; k++) {
        min_dist += localDW->Resize2_LineBuffer[torpedo_ConstP.pooled3[i]] *
          torpedo_ConstP.pooled1[i];
        i += 60;
      }
  ...
  for (j = 0; j < 640; j++) {
    ...
    for (num_colors = 0; num_colors < 80; num_colors++) {
      ...
      min_dist = 0.0;
      for (k = 0; k < 12; k++) {
        min_dist += localB->ColorSpaceConversion1_o2[torpedo_ConstP.pooled4[i] *
          640 + j] * torpedo_ConstP.pooled2[i];
        i += 80;
      }

      localDW->Resize1_IntBuffer[indx] = min_dist;
      indx += 640;
    }
  }
  ...
  for (num_colors = 0; num_colors < 80; num_colors++) {
    memcpy((void *)(&localDW->Resize2_LineBuffer[0]), (void *)
           (&localDW->Resize1_IntBuffer[num_colors * 640]), 640U * sizeof(real_T));
    for (j = 0; j < 60; j++) {
      i = j;
      min_dist = 0.0;
      for (k = 0; k < 22; k++) {
  ...
  for (j = 0; j < 640; j++) {
    ...
    for (num_colors = 0; num_colors < 80; num_colors++) {
      ...
      min_dist = 0.0;
      for (k = 0; k < 12; k++) {
        min_dist += localB->ColorSpaceConversion1_o3[torpedo_ConstP.pooled4[i] *
          640 + j] * torpedo_ConstP.pooled2[i];
        i += 80;
      }

      localDW->Resize2_IntBuffer[indx] = min_dist;
      indx += 640;
    }
  }
  ...
  for (num_colors = 0; num_colors < 80; num_colors++) {
    memcpy((void *)(&localDW->Resize2_LineBuffer[0]), (void *)
           (&localDW->Resize2_IntBuffer[num_colors * 640]), 640U * sizeof(real_T));
    for (j = 0; j < 60; j++) {
      i = j;
      min_dist = 0.0;
      for (k = 0; k < 22; k++) {
  ...
  while (i <= 60) {
    ...
    while ((i <= 60) && (flag == 0)) {
      ...
      while ((j <= 80) && (flag == 0)) {
        ...
        /* '<S10>:1:33' */
        index = 1.0;
        while ((uint32_T)k_0 <= (uint32_T)num_colors) {
          /* '<S10>:1:34' */
          /* '<S10>:1:35' */
          dist = sqrt((rt_pow_snf(localB->ref_colors[(int32_T)k_0 + 49] -
            localB->Resize1[(j - 1) * 60 + (i - 1)], 2.0) * 1.5 + rt_pow_snf
                       (localB->ref_colors[(int32_T)k_0 - 1] - localB->Resize[(j
            - 1) * 60 + (i - 1)], 2.0)) + rt_pow_snf(localB->ref_colors[(int32_T)
            k_0 + 99] - localB->Resize2[(j - 1) * 60 + (i - 1)], 2.0) * 1.5);
          if (min_dist > dist) {
            /* '<S10>:1:36' */
            /* '<S10>:1:37' */
            min_dist = dist;
  ...
  /* '<S9>:1:10' */
  memset((void *)(&localB->LabelMatrix[0]), 0, 307200U * sizeof(real_T));
  ...
  for (i = 0; i < 640; i++) {
    ...
    for (j = 0; j < 480; j++) {
      ...
      /* '<S9>:1:16' */
      for (k_0 = 1.0; k_0 <= localB->num_colors; k_0++) {
        /* '<S9>:1:16' */
        /* '<S9>:1:17' */
        dist = sqrt((rt_pow_snf(localB->ref_colors[(int32_T)k_0 - 1] -
          localB->ColorSpaceConversion1_o1[640 * j + i], 2.0) + rt_pow_snf
                     (localB->ref_colors[(int32_T)k_0 + 49] -
                      localB->ColorSpaceConversion1_o2[640 * j + i], 2.0)) +
                    rt_pow_snf(localB->ref_colors[(int32_T)k_0 + 99] -
          localB->ColorSpaceConversion1_o3[640 * j + i], 2.0));
        if (dist < min_dist) {
          /* '<S9>:1:18' */
          /* '<S9>:1:19' */
          min_dist = dist;
      ...
      /* '<S9>:1:23' */
      localB->LabelMatrix[i + 640 * j] = index;
    }
  }
  ...
  for (k_0 = 1.0; k_0 <= min_dist; k_0++) {
    ...
    for (i = 0; i < 640; i++) {
      ...
      for (j = 0; j < 480; j++) {
        /* '<S13>:1:49' */
        if (localB->LabelMatrix[640 * j + i] != k_0) {
          /* '<S13>:1:50' */
          /* '<S13>:1:51' */
          localB->LogicMatrix[i + 640 * j] = FALSE;
        } else {
          /* '<S13>:1:53' */
          localB->LogicMatrix[i + 640 * j] = TRUE;
        }
      }
    }
    ...
    if (tmp_0 < 50) {
      ...
      for (b_i = 1U; b_i <= TNumBlobs; b_i = (uint8_T)((uint32_T)b_i + 1U)) {
        ...
        localB->blob_hues[tmp_0 - 1] = localB->ref_colors[(int32_T)
          localB->LabelMatrix[((int32_T)dist - 1) * 640 + ((int32_T)b - 1)] - 1];
      }
