...
void * camread_thread(void* in) {
	...
	while (video_already_open) {
		...
		if(video_paused || !(camfd > 0)) sleep(1);
			...
			if(format == YU12){
				if(tenacious_read(camfd, nextframe.y, width * height) < 0) qDebug() << "Camera read() failure";
...
/*	lastframe.y = (unsigned char*)calloc(width * height, sizeof(unsigned char));
	lastframe.cb = (unsigned char*)calloc(width * height / 4, sizeof(unsigned char));
	lastframe.cr = (unsigned char*)calloc(width * height / 4, sizeof(unsigned char));
	nextframe.y = (unsigned char*)calloc(width * height, sizeof(unsigned char));
	nextframe.cb = (unsigned char*)calloc(width * height / 4, sizeof(unsigned char));
	nextframe.cr = (unsigned char*)calloc(width * height / 4, sizeof(unsigned char));
	*/
...
int camread_decode_YUYV_frame(unsigned char* input, camframe* matlabOut, QImage* out){
	//qDebug() << "Pixel 0:0: " << input[0] << input[1] << input[2] << input [3];
	...
	for (i = 0; i < h; ++i){
		for (j = 0; j < w; j+=4){
			int x = j/2;
			int y = i;
			unsigned char Y1 = input[i*w+j];
			frameSum += Y1;
			unsigned char U = input[i*w+j+1];
			unsigned char Y2 = input[i*w+j+2];
			frameSum += Y2;
			unsigned char V = input[i*w+j+3];

			matlabOut->y[x*h+y] = Y1;
			(*matlabOut).y[x*(h+1)+y] = Y2;
			(*matlabOut).cb[(x/2)*h+y/2] = U;
			(*matlabOut).cr[(x/2)*h+y/2] = V;

			if(out){
				unsigned char B1 = (unsigned char)(unsigned int) (1.164*(Y1 - 16) + 2.018*(U - 128));
				unsigned char G1 = (unsigned char)(unsigned int) (1.164*(Y1 - 16) - 0.813*(V - 128) - 0.391*(U - 128));
				unsigned char R1 = (unsigned char)(unsigned int) (1.164*(Y1 - 16) + 1.596*(V - 128));
				unsigned char B2 = (unsigned char)(unsigned int) (1.164*(Y2 - 16) + 2.018*(U - 128));
				unsigned char G2 = (unsigned char)(unsigned int) (1.164*(Y2 - 16) - 0.813*(V - 128) - 0.391*(U - 128));
				unsigned char R2 = (unsigned char)(unsigned int) (1.164*(Y2 - 16) + 1.596*(V - 128));
				/*
