...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_brain_process_check_sum_call( int nlhs, mxArray * plhs[],
    int nrhs, const mxArray * prhs[] );
  if (sf_brain_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_brain_autoinheritance_info( int nlhs, mxArray * plhs[],
    int nrhs, const mxArray * prhs[] );
  if (sf_brain_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_brain_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "brain") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_brain_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_brain_method_dispatcher(SimStruct *S, unsigned int
  chartFileNumber, int_T method, void *data);
extern "C" unsigned int sf_machine_global_method_dispatcher(SimStruct
  *simstructPtr, const char *machineName, unsigned int chartFileNumber, int_T
  method, void *data)
{
  if (!strcmp(machineName,"brain")) {
    return(sf_brain_method_dispatcher(simstructPtr,chartFileNumber,method,data));
  }

  return 0;
}

extern void brain_terminator(void);
...
extern void brain_initializer(void);
extern void brain_register_exported_symbols(SimStruct* S);
extern void brain_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  brain_register_exported_symbols(S);
}
